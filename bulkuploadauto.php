...
$bulkuploadIds = Mage::getModel('bulkuploadcsv/bulkuploadcsv')->getCollection();
		$bulkuploadIds->getSelect()->where('main_table.status = 5');
echo 'Total Submissions: '.$bulkuploadIds->count(); 
$i = 0;
$n = 0;
foreach($bulkuploadIds as $bulkuploadcsv)
{
...
//		$path = Mage::getBaseDir('media') . DS . 'vendorcsv' . DS . 'imgtest' . DS; 
		 ...
		 echo $attributeSetId = Mage::getModel('eav/entity_attribute_set')
			...
			else
			{
				...
				$addproduct = Mage::getModel('catalog/product')
							...
							->setShippingTablerate($shippingcost)
 ...
 $pathreport = Mage::getBaseDir('media') . DS . 'errorcsv' . DS;
                        $targetreport = Mage::getBaseUrl('media').'errorcsv/';
                        $file = $bulkid.'_report.csv';
                        $errorcsvpath = $pathreport.$file;
                    $errorcsvpath1 = $targetreport.$file;
 ...
 $downloaderrorReport =  "<a href=".$errorcsvpath1.">".$file."</a>";

            
					$vars = Array('uploadHtml' =>$uploadHtml,'downloaderrorReport'=>$downloaderrorReport);		
...
} // end of if condition to limit number of uploads in auto mode

echo 'Finished Loop'.$n++;
} //end of for each	
?>
