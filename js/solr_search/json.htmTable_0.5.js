...
function CreateTableView(objArray) {
    ...
    if(array.length>0)
    {
		...
		//	alert(array[i]['payment_method']);
            ...
            str += '<li class="item first"><div class="prCnr"><a class="product-image spriteimg" title="'+array[i]['name']+'" href="http://www.craftsvilla.com/' + array[i]['url_path'] + ' "><img class="lazy" height="160" width="160" alt="'+array[i]['name']+'" src="'+array[i]['image']+'"></a> <p class="shopbrief"><a title="'+array[i]['name']+'" href="http://www.craftsvilla.com/' + array[i]['url_path'] + '">'+array[i]['name']+'</a></p> <div class="vendorname"> <b>By <a href="http://craftsvilla.com/'+array[i]['vendor_url']+'" target="_blank">' + array[i]['vendor'] + '. </a><ul id="socielicon"><li><a href="http://pinterest.com/pin/create/button/?url=www.craftsvilla.com&media='+array[i]['image']+'" class="pin-it-button" count-layout="none" target="_blank"><img border="0" src="http://d1g63s1o9fthro.cloudfront.net/skin/frontend/default/craftsvilla2012/img/PinExt.png"/></a></li>';
			if(array[i]['payment_method'] == 0)
			{
				str += '<li><div class="codimage3" style="float:right">COD<img align="texttop" border="0" src="http://local.craftsvilla.com/doejofinal/skin/frontend/default/craftsvilla2012/images/tickforcod.png"/></div></li></ul>';
			} 
					else{

					str += '<li></li></ul>';
					}
			str += '</b></div><!-- magento original price -->'+special_val+' <!-- magento original price --> <!-- customize price--> <!-- <h3 id="home"></h3><?php// else: ?> <h3 id="home"></h3> <h5></h5> --> <!-- customize price--><div class="clear"></div> </div></li> ';
			
			
                 /*if($_vendorinfo['vendoradmin_payment_methods'][0]!=='cashondelivery'){
		
					str += '<li><div class="codimage3" style="float:right">COD<img align="texttop" border="0" src="'.$this->getSkinUrl('images/tickforcod.png').'"/></div></li></ul>';
					} 
					else{
            ...
            //<!--<div class="wrapper border1">  <div class="indent">  <button type="button" title="Add to Cart" class="button btn-cart fright" onclick="setLocation(\'http://www.craftsvilla.com/checkout/cart/add/uenc/aHR0cDovL3d3dy5jcmFmdHN2aWxsYS5jb20vY2F0YWxvZ3NlYXJjaC9yZXN1bHQvP2Rpcj1kZXNjJnE9dGVycmE,/product/' + array[i]['entity_id'] + '/\')"><span><span>Add to Cart</span></span></button> <div class="products price-box"> <span class="regular-price" id="product-price-' + array[i]['entity_id'] + '"> <span class="price 123">Rs. ' + array[i]['price'] + '</span></span> </div></div></div>-->
            //str += '<li class="item first"><div class="prCnr"><a class="product-image" title="'+array[i]['name']+'" href="http://www.craftsvilla.com/' + array[i]['url_path'] + ' "><img height="200" width="200" alt="'+array[i]['name']+'" src="http://cdnkribha1.s3.amazonaws.com/catalog/product/cache/1/small_image/200x200/9df78eab33525d08d6e5fb8d27136e95'+trim(array[i]['image'])+'"></a> <h1><a title="'+array[i]['name']+'" href="http://www.craftsvilla.com/' + array[i]['url_path'] + '">'+array[i]['name']+'</a></h1> <!--<p class="vendorname">By: <a href="http://www.craftsvilla.com/" target="_blank">' + array[i]['vendor'] + '</a></p>--> <div class="vendorname"> <b>By ' + array[i]['vendor'] + '. </b></div><!-- magento original price --><div class="products price-box"> <span id="product-price-' + array[i]['entity_id'] + '" class="regular-price"> <span class="price 123">Rs. ' + array[i]['price'] + '</span> </span> </div> <!-- magento original price --> <!-- customize price--> <!-- <h3 id="home"></h3><?php// else: ?> <h3 id="home"></h3> <h5></h5> --> <!-- customize price--> <div class="clear"></div> <!--<div class="wrapper border1">  <div class="indent">  <button type="button" title="Add to Cart" class="button btn-cart fright" onclick="setLocation(\'http://www.craftsvilla.com/checkout/cart/add/uenc/aHR0cDovL3d3dy5jcmFmdHN2aWxsYS5jb20vY2F0YWxvZ3NlYXJjaC9yZXN1bHQvP2Rpcj1kZXNjJnE9dGVycmE,/product/' + array[i]['entity_id'] + '/\')"><span><span>Add to Cart</span></span></button> <div class="products price-box"> <span class="regular-price" id="product-price-' + array[i]['entity_id'] + '"> <span class="price 123">Rs. ' + array[i]['price'] + '</span></span> </div></div></div>--></div><iframe frameborder="0" scrolling="no" title="Like it!" class="fblikebtn" allowtransparency="true" src="http://www.facebook.com/plugins/like.php?layout=button_count&amp;href=http://www.craftsvilla.com/' + array[i]['url_path'] + '"></iframe> <img title="Like it!" alt="" class="fbthumbic" src="http://www.craftsvilla.com/skin/frontend/default/craftsvilla/images/fblike.gif"></li>';
