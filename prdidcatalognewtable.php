...
foreach($readPrdQuery as $resultPrdcts)
{
	//var_dump($resultPrdcts);exit ;
		...
		$readCrdQuery = mysql_query($_productC)->fetchAll();
			$value = array();
			$k = 0;
			$valueId[0] ='';
			$valueId[1] ='';
		...
		$readQuery = "select `entity_id` from `catalog_product_craftsvilla2` where `entity_id` = '".$prdId."'";
...
//			var_dump($dataCodd);exit;
		...
		//echo $id[0]; exit;
			if(!$id[0])
			{
				...
				$import = "INSERT INTO `catalog_product_craftsvilla2`(`entity_id`,`entity_type_id`,`attribute_set_id`,`type_id`,`sku`,`created_at`,`updated_at`,`has_options`,`required_options`, `price`, `final_price`, `minimal_price`, `min_price`, `max_price`, `tier_price`,`discount`,`shippingtime`, `website_id`, `customer_group_id`, `visibility`, `category_id1`, `category_id2`, `category_id3`,`category_id4`, `store_id`, `cod`, `zip`, `udropship_vendor`) 
														  VALUES ('".$prdId."','".$resultPrdcts['entity_type_id']."','".$resultPrdcts['attribute_set_id']."','".$resultPrdcts['type_id']."','".mysql_real_escape_string($resultPrdcts['sku'])."','".$resultPrdcts['created_at']."','".$resultPrdcts['updated_at']."','".$resultPrdcts['has_options']."','".$resultPrdcts['required_options']."','".$resultPrdcts['price']."','".$resultPrdcts['final_price']."','".$resultPrdcts['minimal_price']."','".$resultPrdcts['min_price']."','".$resultPrdcts['max_price']."','".$resultPrdcts['tier_price']."','".$finalDiscPrice."','".$exactIntervalday."','".$resultPrdcts['website_id']."','".$resultPrdcts['customer_group_id']."','".$resultPrdcts['visibility']."','".$categoryId1."','".$categoryId2."','".$categoryId3."','".$categoryId4."','".$resultPrdcts['store_id']."','".$codMthd."','".$zip."','".$vendor['value']."')";
...
$db->query($import);
				...
				$import = "UPDATE `catalog_product_craftsvilla2` SET `entity_id`='".$prdId."',`entity_type_id` = '".$resultPrdcts['entity_type_id']."',`attribute_set_id` = '".$resultPrdcts['attribute_set_id']."',`type_id` = '".$resultPrdcts['type_id']."',`sku` = '".mysql_real_escape_string($resultPrdcts['sku'])."',`created_at` = '".$resultPrdcts['created_at']."',`updated_at` = '".$resultPrdcts['updated_at']."', `has_options` = '".$resultPrdcts['has_options']."',`required_options` = '".$resultPrdcts['required_options']."',`price`='".$resultPrdcts['price']."',`final_price`='".$resultPrdcts['final_price']."',`minimal_price`='".$resultPrdcts['minimal_price']."',`min_price`='".$resultPrdcts['min_price']."',`max_price`='".$resultPrdcts['max_price']."',`tier_price`='".$resultPrdcts['tier_price']."',`discount`='".$finalDiscPrice."',`shippingtime`='".$exactIntervalday."',`website_id`='".$resultPrdcts['website_id']."',`customer_group_id`='".$resultPrdcts['customer_group_id']."',`visibility`='".$resultPrdcts['visibility']."',`category_id1`='".$categoryId1."',`category_id2`='".$categoryId2."',`category_id3`='".$categoryId3."',`category_id4`='".$categoryId4."',`store_id`='".$resultPrdcts['store_id']."',`cod`='".$codMthd."',`zip`='".$zip."',`udropship_vendor`='".$vendor['value']."'
				WHERE `catalog_product_craftsvilla2`.`entity_id` ='".$prdId."'";
				//mysql_query($import) or die(mysql_error());
