    ...
    implements Mage_Eav_Model_Entity_Interface
{
    ...
    {
      // Commented on Date : 27-02-2014 For reduce the query on product view page------START  
		//$select = $this->_getReadAdapter()->select()->union($selects);
		$select = $this->_getReadAdapter()->select()->union($selects,Zend_Db_Select::SQL_UNION_ALL);
        return $select;
	// Commented on Date : 27-02-2014 For reduce the query on product view page------END	
    }
