    ...
    extends Mage_Catalog_Model_Resource_Eav_Mysql4_Collection_Abstract
{
	...
	protected $_paramsListCraftsvilla = array();
	protected $_orderListCraftsvilla = array();
	protected $_curpageListCraftsvilla = 1;
    /**
     ...
     * @var array
     */
    protected $_priceDataFieldFilters = array('min_discount');
    ...
    protected function _initSelect()
    {
        if ($this->isEnabledFlat()) {
          ...
          //Coomented the previous table name and new table name in belo..name as  'catalog_proudct_craftsvilla on dated 11-01-2014' 
		    //$this->getSelect()->from(array('e'=>$this->getEntity()->getEntityTable()));
			$this->getSelect()->from(array('e'=>'catalog_product_craftsvilla3'));
        }
        return $this;
    }
...
//Added by dileswar on dated 05-03-2014 to remove DISTINCT from catalog query........
...
// Added below condition on dated 21-02-2014 for block name search on list page		
        ...
        if ($attribute == 'price' && $storeId != 0) {
            $this->addPriceData();
            $this->getSelect()->order("e.min_price {$dir}");
		...
		{
			 //$this->addPriceData();
            $this->getSelect()->order("e.discount {$dir}");
			//echo $this->getSelect()->__toString();exit;
            return $this;
		}
		
		if($attribute == 'shippingtime' && $storeId != 0)
		{
			 //$this->addPriceData();
            $this->getSelect()->where("e.shippingtime>0.5")->order("e.shippingtime {$dir}");
			//echo $this->getSelect()->__toString();exit;
            return $this;
		}
    ...
    protected function _productLimitationJoinStore()
    {
        ...
        if (!$hasColumn) {
            $this->getSelect()->columns('visibility', 'e');
        }
        ...
        $whereCond = join(' OR ', array(
            $this->getConnection()->quoteInto('e.visibility IN(?)', $filters['visibility']),
    ...
    protected function _productLimitationJoinPrice()
    {
	  ...
	  // Changed by Manoj on 06-07-2013 to enable Flat tables to be used for category and shop page viewing on frontend
            ...
            $this->getSelect()->join(
                ...
                $joinCond,
                array('price', 'tax_class_id', 'final_price', 'minimal_price' => $minimalExpr ,'min_price', 
                    'tier_price')
            );
    ...
    protected function _applyProductLimitations()
    {
        ...
        $conditions = array(
		// changed the primary key id from product_id to entity_id
            //'cat_index.product_id=e.entity_id',
			//'cat_index.entity_id=e.entity_id',
            //$this->getConnection()->quoteInto('cat_index.store_id=?', $filters['store_id'])
        );
        if (isset($filters['visibility']) && !isset($filters['store_table'])) {
            $conditions[] = $this->getConnection()
                ->quoteInto('e.visibility IN(?)', $filters['visibility']);
        }
        $conditions[] = $this->getConnection()
            ->quoteInto('(e.category_id1=? OR e.category_id2=? OR e.category_id3=? OR e.category_id4=?)', $filters['category_id']);
  // Changed by Manoj on 06-07-2013 to enable Flat tables to be used for category and shop page viewing on frontend
		...
		$this->getSelect()->where($joinCond);
//Below 4 line added on dated 05-03-2014 for reduce category query by Dileswar		
		
		/*$wherePart = $this->getSelect()->getPart(Zend_Db_Select::WHERE);
		if(!isset($wherePart[0]))
			{
        ...
        if (isset($fromPart['e'])) {
			// $fromPart['e']['joinCondition'] = $joinCond;
			$this->getSelect()->setPart(Zend_Db_Select::FROM, $fromPart);
		}
        else {
            $this->getSelect()->join(
        // added new table to join the tables in one table ..dated 08-01-2014    
			    //array('cat_index' => $this->getTable('catalog/category_product_index')),
					array('e' => 'catalog_product_craftsvilla3'),
                $joinCond,
...
//echo $this->getSelect()->__toString();
	...
	public function setCvFilter($key,$val){
		
		if(($key == 'cod') && ($val == 'showcod')){
			$valCod = 1;
			$this->getSelect()->where("e.".$key." = '".$valCod."'");
			$this->_paramsListCraftsvilla['cod'] = $val;		
		}	
		
		//echo $this->getSelect()->__toString();exit;
		}
		
    public function setCvzipFilter($key,$val){
		//$this->getSelect()->where("e.".$key." = '".$val."'");
		$rest = substr($val, 0, 4);

		$this->getSelect()->where("e.".$key."  like '%".$rest."%'");
		$this->_paramsListCraftsvilla['zip'] = $val;
		//echo $this->getSelect()->__toString();exit;
			}
	public function setCvminpriceFilter($key1,$key2,$val1,$val2){
	
		if($val2=='Above Rs. 5000')
		{
			$this->getSelect()->where("e.min_price Between '".$val1."' AND  100000");
		}
		else
		{
		 $this->getSelect()->where("e.min_price Between ".mysql_escape_string($val1)." AND ".mysql_escape_string($val2));
		 //echo $this->getSelect()->__toString();exit;
		}
		
		//echo $this->getSelect()->__toString();exit;

		$this->_paramsListCraftsvilla['min_price'] = $val1;
		$this->_paramsListCraftsvilla['max_price'] = $val2;
		
	}
	public function setCvmindiscountFilter($val1,$val2){

		 $this->getSelect()->where("e.discount Between ".mysql_escape_string($val1)." AND ".mysql_escape_string($val2));
		 //echo $this->getSelect()->__toString();exit;
		$this->_paramsListCraftsvilla['min_discount'] = $val1;
		$this->_paramsListCraftsvilla['max_discount'] = $val2;
		}
	public function setColorFilter($selectcolor){

		$this->getSelect()->where("e.color = '".$selectcolor."'");
		$this->_paramsListCraftsvilla['color'] = $selectcolor;
		 }

//added on dated 30-07-2014
	public function loadProductListCv()
	{
		$vendor = Mage::helper('umicrosite')->getCurrentVendor();
		
		$mindiscCv = $this->_paramsListCraftsvilla['min_discount'];
		$maxdiscCv = $this->_paramsListCraftsvilla['max_discount'];
		if(!is_numeric($mindiscCv)&&($mindiscCv)) $mindiscCv = 10;
		if(!is_numeric($maxdiscCv)&&($maxdiscCv)) $maxdiscCv = 100;
		$minpriceCv = $this->_paramsListCraftsvilla['min_price'];
		$maxpriceCv = $this->_paramsListCraftsvilla['max_price'];
		if(!is_numeric($minpriceCv)&&($minpriceCv)) $minpriceCv = 100;
		if(!is_numeric($maxpriceCv)&&($maxpriceCv)) $maxpriceCv = 100000;
		$colorCv = $this->_paramsListCraftsvilla['color'];
		$zipCv = $this->_paramsListCraftsvilla['zip'];
		if(!is_numeric($zipCv)&&($zipCv)) $zipCv = 400101;
		$codCv = $this->_paramsListCraftsvilla['cod'];
		$categoryidCv = $this->_productLimitationFilters['category_id'];
		$offsetCv = ($this->_curpageListCraftsvilla-1)*120;
		if(!is_numeric($offsetCv)) $offsetCv = 0;
                if($offsetCv < 0) $offsetCv = 0;
		$priceorderCv = $this->_orderListCraftsvilla['price'];
		$shipingorderCv = $this->_orderListCraftsvilla['shippingtime'];
		$discountorderCv = $this->_orderListCraftsvilla['discount'];
		$whereSelect = '';
		//$this->_itemsById = array();
		if($mindiscCv && $maxdiscCv) $whereSelect = " AND discount > ".$mindiscCv." AND discount < ".$maxdiscCv;
		if($vendor) $whereSelect = " AND udropship_vendor = ".$vendor->getId();
		if($minpriceCv && $maxpriceCv)
			{
				
					$whereSelect .= " AND min_price > ".$minpriceCv." AND min_price < ".$maxpriceCv;
					
			}
		if($colorCv)
			{
				
					$whereSelect .= " AND color = '".mysql_escape_string($colorCv)."'";
					
			}
		if($zipCv)
			{
					$whereSelect .= " AND zip LIKE '%".substr($zipCv,0,4)."%'";
			}
		if($codCv == 'showcod')
			{
				$whereSelect .= " AND cod = 1";
			}
			$categoryselectCv = " WHERE visibility IN (4) AND (category_id1 =".$categoryidCv." OR category_id2 = ".$categoryidCv." OR category_id3 = ".$categoryidCv." OR category_id4= ".$categoryidCv.")";

		if($priceorderCv)
		{
			$orderSelect = " ORDER BY min_price ".$priceorderCv;
		}
		elseif($shipingorderCv)
		{
			$orderSelect = " ORDER BY shippingtime ".$shipingorderCv;
		}
		elseif($discountorderCv)
		{
			$orderSelect = " ORDER BY discount ".$discountorderCv;
		}
		else
		{
			$orderSelect = " ORDER BY entity_id DESC ";		
		}
		$offsetSelect = " LIMIT 120 OFFSET ".$offsetCv;	
		$selectCollectionQuery = "SELECT `entity_id` FROM `catalog_product_craftsvilla3` ".$categoryselectCv . $whereSelect.  $orderSelect . $offsetSelect;
		//$selectCollectioncountQuery = "SELECT count(*) AS countcv FROM `catalog_product_craftsvilla3` ".$categoryselectCv . $whereSelect.  $orderSelect ;
		$selectCollectioncountQuery = "SELECT count(*) AS countcv FROM `catalog_product_craftsvilla3` ".$categoryselectCv . $whereSelect ;
		$getconnect = Mage::getSingleton('core/resource')->getConnection('core_read');
		$getResult = $getconnect->query($selectCollectionQuery)->fetchAll();
		$getcount = $getconnect->query($selectCollectioncountQuery)->fetchAll();
		$getconnect->closeConnection();
		$getcountResult = $getcount[0]['countcv'];
		$idArray = array();
		$k = sizeof($getResult);
		$j = 0;
		foreach($getResult as $_getResult){
			//$idArray[$k-$j-1] = $_getResult['entity_id'];
			$idArray[] = $_getResult['entity_id'];
			$j++;
			}
	//echo '<pre>';print_r($idArray);
	if($idArray){	
	$_prdouct_list_a1 = Mage::helper('catalog/product')->loaddataarray($idArray);	
	$prdouct_list_a = $this->decorateArrayCv($idArray,$_prdouct_list_a1);
}
	//krsort($prdouct_list_a);
//echo '<pre>';print_r($prdouct_list_a);exit;
//foreach($getResult as $_getResult){
						
		foreach($prdouct_list_a as $_prdouct_list_a){	
			$key = $_prdouct_list_a['entity_id'];
			//$_prdouct = Mage::helper('catalog/product')->loadnewlistcv($_getResult['entity_id']);
            $_prdouct = Mage::helper('catalog/product')->getLoadedProductCV($_prdouct_list_a);
			$this->_itemsById[$key][0] = $_prdouct;
			$this->_items[$key] = $_prdouct;
			
		}
		$this->_setIsLoaded(true);
		$this->_totalRecords = $getcountResult;
		//echo '<pre>';print_r($this);exit; 
		
	}

public function decorateArrayCv($idArray,$_prdouct_list_a){
		$decoratedArray = array();
		$i = 0;
		foreach($idArray as $_idArray)
		{
			$decoratedArray[$i]['productData'] = $_prdouct_list_a[$_idArray];
			$decoratedArray[$i]['entity_id'] = $_idArray;
			$i++;
		}
	...
	public function setOrderCraftsvilla($order,$dir)
	{
		$this->_orderListCraftsvilla[$order]=$dir;
	}
	public function setcurpageCraftsvilla($pageId)
	{
		$this->_curpageListCraftsvilla = $pageId;
	}
