...
$categoryIdcv = $this->fetchProductCategory($entiyPrdId);
			//$product_vendor_name = $categorynvendor_arr[1];
			$arryCat = array('6'=>'Jewelry','74' =>'Sarees','5'=>'Home Decor','1070'=>'Home Furnishing','4'=>'Clothing','9'=>'Bags','8'=>'Accessories','284'=>'Bath & Beauty','69'=>'Food & Health','1114'=>'Rakhi Gifts');
$product_category_name = 'Online Shopping';
			if($categoryIdcv)
	...
	$product->setUdropshipVendor($vendorIdCv);
...
//new function added for list/catalog page
public function loadnewshort($entiyPrdId){
...
//GET VENDOR ID / check international ship allowed or not
	$attrquery = "SELECT `attribute_id`,`value` FROM `catalog_product_entity_int` WHERE `entity_id` = '".$entiyPrdId."' AND `attribute_id` IN ('531')";
	$resultAttrInt = $readId->query($attrquery)->fetchAll();
	$checkAvailableInt = array();	// 1- mean available 0- not availabel
	foreach($resultAttrInt as $_resultAttrInt){$checkAvailableInt[$_resultAttrInt['attribute_id']] = $_resultAttrInt['value'];}
	$vendorIdCv = $checkAvailableInt['531'];
...
$catentityDecimalQuery = "SELECT `attribute_id`,`value` FROM `catalog_product_entity_decimal` WHERE `entity_id` = '".$entiyPrdId."' AND `attribute_id` IN('60','61') ";
$resultcatdecimal	= $readId->query($catentityDecimalQuery)->fetchAll();
...
//product name, Meta Desc
$catentityvarcharQuery = "SELECT `attribute_id`,`value` FROM `catalog_product_entity_varchar` WHERE `entity_id` = '".$entiyPrdId."' AND `attribute_id` IN('56','82') ";
$resultcatvarchar	= $readId->query($catentityvarcharQuery)->fetchAll();
foreach($resultcatvarchar as $_resultcatvarchar){$cattext[$_resultcatvarchar['attribute_id']] = $_resultcatvarchar['value'];}
...
$product_category_name = 'Online Shopping';
	...
	public function loaddataarray($entityid_a)
	{
	$entity_data_a = array();	
	$baseCurrencyCode = Mage::app()->getStore()->getBaseCurrencyCode();
	$currentCurrencyCode = Mage::app()->getStore()->getCurrentCurrencyCode();
	$readId = Mage::getSingleton('core/resource')->getConnection('core_read');
	//$entityid_a = array('26089','26080');
	$_entityid_a = rtrim(implode(',', $entityid_a), ',');
	$lastEntityId = '';
//print_r($entityid_a); exit;
		
		//for product images
		$prdImageQuery = "SELECT `entity_id`,`value_id`,`value` as `file` FROM `catalog_product_entity_media_gallery` WHERE `entity_id` IN (".$_entityid_a.")";
		$resultImage = $readId->query($prdImageQuery)->fetchAll();
		//echo '<pre>';print_r($resultImage);exit;		
		$_resultImage = array();
		
		foreach($resultImage as $_resultimg)
		{
			if($_resultimg['entity_id'] != $lastEntityId){
			//echo '<pre>';print_r($_resultimg);exit;
			$entity_data_a[$_resultimg['entity_id']]['image']['images'][0]['file'] = $_resultimg['file'];
			$entity_data_a[$_resultimg['entity_id']]['image']['images'][0]['value_id'] = $_resultimg['value_id'];
			$entity_data_a[$_resultimg['entity_id']]['image']['values']=array();
			$i = 0;

			$entity_data_a[$_resultimg['entity_id']]['image']['images'][$i]['label'] = '';
			$entity_data_a[$_resultimg['entity_id']]['image']['images'][$i]['position'] = $i+1;
			$entity_data_a[$_resultimg['entity_id']]['image']['images'][$i]['disabled'] = 0;
			$entity_data_a[$_resultimg['entity_id']]['image']['images'][$i]['label_default'] = '';
			$entity_data_a[$_resultimg['entity_id']]['image']['images'][$i]['position_default'] = $i+1;
			$entity_data_a[$_resultimg['entity_id']]['image']['images'][$i]['disabled_default'] = 0;
			$i++;
			}
			$lastEntityId = $_resultimg['entity_id'];
		}

	
	//check inventory `is_in_stock`
	$catStockItem = "SELECT `product_id`,`is_in_stock` FROM `cataloginventory_stock_item` WHERE `product_id` IN (".$_entityid_a.")";
	$resultcatStockItem = $readId->query($catStockItem)->fetchAll();
	$checkstocks = $resultcatStockItem['is_in_stock'];
		foreach($resultcatStockItem as $_resultcatStockItem)
		{
			$entity_data_a[$_resultcatStockItem['product_id']]['is_in_stock'] = $_resultcatStockItem['is_in_stock'];
		}
	//type_id & sku
	$typeIdnskuquery = "SELECT `entity_id`,`type_id`,`sku` FROM `catalog_product_entity` WHERE `entity_id` IN (".$_entityid_a.")";
	$resultofskunidtype = $readId->query($typeIdnskuquery)->fetchAll();
	
	foreach($resultofskunidtype as $_resultofskunidtype){
		$entity_data_a[$_resultofskunidtype['entity_id']]['type_id'] = $_resultofskunidtype['type_id'];
		$entity_data_a[$_resultofskunidtype['entity_id']]['sku'] = $_resultofskunidtype['sku'];
		}

//price,special price,shipping cost, internationalshipping price

$catentityDecimalQuery = "SELECT `entity_id`,`attribute_id`,`value` FROM `catalog_product_entity_decimal` WHERE `entity_id` IN (".$_entityid_a.") AND `attribute_id` IN('60','61','535','643') ";
$resultcatdecimal	= $readId->query($catentityDecimalQuery)->fetchAll();

foreach($resultcatdecimal as $_resultcatdecimal){
	if ($_resultcatdecimal['attribute_id'] == 60 ){ 
		if($currentCurrencyCode !== 'INR')
		$entity_data_a[$_resultcatdecimal['entity_id']]['price'] = $_resultcatdecimal['value']*1.5;
		else 
		$entity_data_a[$_resultcatdecimal['entity_id']]['price'] = $_resultcatdecimal['value'];
	}

	if($_resultcatdecimal['attribute_id'] == 61){ 
		if($currentCurrencyCode !== 'INR')
		$entity_data_a[$_resultcatdecimal['entity_id']]['special_price'] = $_resultcatdecimal['value']*1.5;
		else 
		$entity_data_a[$_resultcatdecimal['entity_id']]['special_price'] = $_resultcatdecimal['value'];
	}
	if($_resultcatdecimal['attribute_id'] == 535){ 
		$entity_data_a[$_resultcatdecimal['entity_id']]['shippingcost'] = $_resultcatdecimal['value'];
		}
	if($_resultcatdecimal['attribute_id'] == 643){ 
		$entity_data_a[$_resultcatdecimal['entity_id']]['intershippingcost'] = $_resultcatdecimal['value'];//655 live id 
		}
}
//-----------------------------------			
...
$catentityDatetimeQuery = "SELECT `entity_id`,`attribute_id`,`value` FROM `catalog_product_entity_datetime` WHERE `entity_id` IN (".$_entityid_a.") AND `attribute_id` IN('62','63') ";
$resultcatdatetime	= $readId->query($catentityDatetimeQuery)->fetchAll();

foreach($resultcatdatetime as $_resultcatdatetime){
	if($_resultcatdatetime['attribute_id'] == 62){ 
		$entity_data_a[$_resultcatdatetime['entity_id']]['special_from_date'] = $_resultcatdatetime['value'];
		}
	if($_resultcatdatetime['attribute_id'] == 63){ 
		$entity_data_a[$_resultcatdatetime['entity_id']]['special_to_date'] = $_resultcatdatetime['value'];
		}
}
//-----------------------------------			
//product name,url_key
$catentityvarcharQuery = "SELECT `entity_id`,`attribute_id`,`value` FROM `catalog_product_entity_varchar` WHERE `entity_id` IN (".$_entityid_a.") AND `attribute_id` IN ('56','82') ";
$resultcatvarchar	= $readId->query($catentityvarcharQuery)->fetchAll();
foreach($resultcatvarchar as $_resultcatvarchar)
	{
	 $entity_data_a[$_resultcatvarchar['entity_id']]['name'] = $_resultcatvarchar['value'];
	 $entity_data_a[$_resultcatvarchar['entity_id']]['url_key'] = $_resultcatvarchar['value'];
	}
	
	
//-----------------------------------	
$vendorIdQuery = "SELECT `entity_id`,`attribute_id`,`value` FROM `catalog_product_entity_int` WHERE `entity_id` IN (".$_entityid_a.") AND `attribute_id` = '531'";
	$resultAttrInt = $readId->query($vendorIdQuery)->fetchAll();
	foreach($resultAttrInt as $_resultAttrInt){

	$entity_data_a[$_resultAttrInt['entity_id']]['udropship_vendor'] = $_resultAttrInt['value'];}

return $entity_data_a;
			
	}
public function getLoadedProductCV($_prdouct_list_a){
...
$product = Mage::getModel('catalog/product');
	...
	$product->setTypeId($_prdouct_list_a['productData']['type_id']);
	$product->setSku($_prdouct_list_a['productData']['sku']);
	$product->setProductName($_prdouct_list_a['productData']['name']);
	$product->setName($_prdouct_list_a['productData']['name']);
	$product->setMediaGallery($_prdouct_list_a['productData']['image']);
	$product->setImage($_prdouct_list_a['productData']['image']['images'][0]['file']);
	$product->setSmallImage($_prdouct_list_a['productData']['image']['images'][0]['file']);
    $product->setThumbnail($_prdouct_list_a['productData']['image']['images'][0]['file']);
	$product->setPrice($_prdouct_list_a['productData']['price']);
	$product->setSpecialPrice($_prdouct_list_a['productData']['special_price']);
	$product->setSpecialFromDate($_prdouct_list_a['productData']['special_from_date']);
	$product->setSpecialToDate($_prdouct_list_a['productData']['special_to_date']);
	$product->setUdropshipVendor($_prdouct_list_a['productData']['udropship_vendor']);
	$product->setIsInStock($_prdouct_list_a['productData']['is_in_stock']);
	$product->setUrlKey($_prdouct_list_a['productData']['url_key']);
	$product->setIsSalable(1);
	$product->setStatus(1);
	return $product;
}
