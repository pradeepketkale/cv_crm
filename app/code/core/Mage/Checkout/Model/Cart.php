    ...
    public function updateItems($data)
    {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            if ($qty > 0) {
				...
				$oldqty = $item->getQty();
				$item->setQty($qty);
				///Commmented By Dileswar(On Dated 05-02-2013)-------------////////
            ...
            $customUpdate = '';
            $customD = $itemInfo['custom'];
            //if(isset($itemInfo['custom'])){
            	$customOld = $item->getAdditionalData();
            	if($customD ==''){
            	$customUpdate = $item->setAdditionalData($customOld);
            	}
            	else{
            	$customUpdate = $item->setAdditionalData($customD);
            	}
        }
