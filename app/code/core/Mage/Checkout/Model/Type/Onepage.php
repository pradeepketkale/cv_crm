...
class Mage_Checkout_Model_Type_Onepage
{
    ...
    protected function _prepareNewCustomerQuote()
    {
		...
		$customer->setPasswordHash($customer->hashPassword($newPassword));
		//$this->save();
    ...
    public function saveOrder()
    {
		...
		$custexistingid = $this->_customerEmailExistsCraftsvilla($email);
		...
		//if($customerexistFlag){ $this->get$custexistingid}
        if ($isNewCustomer) {
            try {
...
public function _customerEmailExistsCraftsvilla($email)
    {
	$read = Mage::getSingleton('core/resource')->getConnection('core_read');        
	$customerQuery = "SELECT `entity_id` FROM `customer_entity` WHERE `email` = '".$email."' ";
	$resultcust = $read->query($customerQuery)->fetch();
	if($resultcust['entity_id']){ 
		return $resultcust['entity_id'];
	}		
	return false;
    }
/*public function _updatecustomerEmailExistsCraftsvilla($id)
    {
	$read = Mage::getSingleton('core/resource')->getConnection('core_read');        
	$customerQuery = "SELECT `entity_id` FROM `customer_entity` WHERE `email` = '".$email."' ";
	$resultcust = $read->query($customerQuery)->fetch();
