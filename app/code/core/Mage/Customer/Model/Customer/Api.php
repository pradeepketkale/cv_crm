...
class Mage_Customer_Model_Customer_Api extends Mage_Customer_Model_Api_Resource {
	...
	public function items($filters) {
		...
		$collection->getSelect()->where("SapSync = '0'")->limit(3000);
		if (is_array($filters)) {
			try {
	...
	public function syncCustomerSBO($customerId, $flag) {
		try {
			$conn = Mage::getSingleton('core/resource')->getConnection('customer_write');
			$data = array('SapSync' => $flag);
			$where = "entity_id = '" . $customerId . "'";
			$conn->update(Insync_Sap_Model_Config::getTableSapContext('customer_entity'), $data, $where);

			$data_address = array('SapSync' => $flag);
			$where = "parent_id = '" . $customerId . "'";
			$conn->update(Insync_Sap_Model_Config::getTableSapContext('customer_address_entity'), $data_address, $where);
