	...
	public function items($customerId) {
		...
		foreach ($customer->getAddresses() as $address) {
			...
			foreach ($this->getAllowedAttributes($address) as $attributeCode => $attribute) {
				...
				/**
				 * Customization on downloading of Street Address Lines as an Array
				 */
				/* if ($attributeCode == 'street') {
				  $counter = Mage::getStoreConfig('customer/address/street_lines');
				  $arrStreet = array();

				  if ($counter) {
				  for ($i = 1; $i <= $counter; $i++) {
				  $arrStreet[$i] = $address->getStreet($i);
				  }
				  }
				  else {
				  $arrStreet[] = $data[$attributeCode];
				  }

				  $row[$attributeCode] = $arrStreet;
				  } */
			}
			...
			if ($row['region_id'] != NULL && array_key_exists('region_id', $row)) {
				$regionObj = new Mage_Directory_Model_Region();
				$regionObj->load($row['region_id']);
				if ($regionObj->getData('code') != NULL) {
					$row['region'] = $row['region'] . '@' . $regionObj->getData('code');
				}
				unset($regionObj);
			}
