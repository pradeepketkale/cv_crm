...
class Mage_Customer_Model_Address_Api_V2 extends Mage_Customer_Model_Address_Api {
	...
	public function create($customerId, $addressData) {
		...
		/**
		 * Start of Customization for fetching proper Customer Address Region ID
		...
		foreach ($this->getAllowedAttributes($address) as $attributeCode => $attribute) {
			if (isset($addressData->region) && !empty($addressData->region)) {
			...
			if ('region_id' == $attributeCode && (!isset($addressData->$attributeCode) || empty($addressData->$attributeCode))) {
				$objSapConfig = new Insync_Sap_Model_Config();
				$address->setData($attributeCode, $objSapConfig->getCountryRegionId($regionCode, $countryCode));
				continue;
			}
