...
class Craftsvilla_Shipmentpayout_Adminhtml_ImportutrController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('sales/shipmentpayout/importutr')
            ->_addContent($this->getLayout()->createBlock('shipmentpayout/adminhtml_import_formutr'))
            ->renderLayout();
    }
     ...
     * Import Action
     */
    public function importutrAction()
    {
    ...
    protected function _importUtrFile($fileName)
    {
        ...
        foreach ($csvData as $k => $v) {
           ...
           if($orderId){
                $queryShipmentUtr = "update shipmentpayout set citibank_utr='".$utrNumber."',shipmentpayout_update_time='".$date."' WHERE order_id = '".$orderId."'";  
           ...
           $this->_getSession()->addSuccess($this->__('Orders UTR Numbers are successfully saved!!!'));
