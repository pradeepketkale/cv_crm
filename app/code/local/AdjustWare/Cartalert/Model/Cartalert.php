 ...
 */
?>
<?php if(Aitoc_Aitsys_Abstract_Service::initSource(__FILE__,'AdjustWare_Cartalert')){ BjZDMegyaeahogoI('600d052c5cf075cb6de50984d647848a'); ?><?php
/**
    ...
    public function preprocess($store=null){
        ...
        for ($i=0, $n=sizeof($prod); $i<$n; $i+=2){
            ...
            if(in_array($product->getStatus(),$status)&&in_array($product->getVisibility(),$visibility)&&$product->isSaleable())
            {
                ...
                {
                    $imageTag ='<img src="'.Mage::helper('catalog/image')->init($product ,
                    'small_image')->resize(75).'" border="0" style="float:left;padding:0 9px 9px 0"/>';
                }
                $items[$prod[$i]] = '<div style="padding:8px 0 8px 8px; overflow:hidden;background:#EFEFEF;border-radius:5px;margin-bottom:10px;"><a href="'.$url.'">'.$imageTag.'</a><a href="'.$url.'" style="color:#666;text-decoration:none;">'.$name.'</a></div>'; //to omit duplicates
            }        
        }
