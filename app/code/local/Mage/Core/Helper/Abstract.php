...
abstract class Mage_Core_Helper_Abstract {
    ...
    /**
     *   Category Sorting by numeric, alphabates, special chars, Rs.
     *
     *  @param    array $arr
     *  @return	  array
     */

    public function fieldSort($results) {
        $results_alpha = array();
        $results_sp = array();
        $results_sorted = array();
        foreach ($results as $key => $val) {
            $rspos = 1;
            $rspos = strpos($val['name'], "Rs.");
            if ($rspos === 0) {
                $results_sp[$key] = $results[$key];
            } else {
                $ll = "";
                $ll = strtolower(substr($val['name'], 0, 1));
                if ($ll != "") {
                    if (preg_match('/[a-z0-9]/', $ll) == 1) {
                        $results_alpha[$key] = $results[$key];
                    } else {
                        $results_sp[$key] = $results[$key];
                    }
                } else {
        ...
        $results_sorted = array_merge($results_alpha, $results_sp);
