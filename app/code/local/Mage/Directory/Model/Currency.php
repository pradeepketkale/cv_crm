    ...
    */
    ...
    public function convert($price, $toCurrency=null)
    {
        ...
        elseif ($rate = $this->getRate($toCurrency)) {
            //return $price*$rate;
			return ceil($price*$rate); //added for round price
        }

        //throw new Exception(Mage::helper('directory')->__('Undefined rate from "%s-%s".', $this->getCode(), $toCurrency->getCode()));
		throw new Exception(Mage::helper('directory')->__('There is an error in currency conversion.'));
    }
    ...
    public function format($price, $options=array(), $includeContainer = true, $addBrackets = false)
    {
		...
		//added my mehul 30-08-11
		($this->getCurrencyCode() == "INR") ? $precision = 0 : $precision = 2;
    ...
    public function formatPrecision($price, $precision, $options=array(), $includeContainer = true, $addBrackets = false)
    {
        ...
        if ($includeContainer) {
            return '<span class="price 123">' . ($addBrackets ? '[' : '') . $this->formatTxt($price, $options) . ($addBrackets ? ']' : '') . '</span>';
        }
