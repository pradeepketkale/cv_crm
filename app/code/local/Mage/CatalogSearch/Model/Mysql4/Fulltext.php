...
class Mage_CatalogSearch_Model_Mysql4_Fulltext extends Mage_Core_Model_Mysql4_Abstract {
    ...
    protected function _getSearchableProducts($storeId, array $staticFields, $productIds = null, $lastProductId = 0, $limit = 100) {
        ...
        $select = $this->_getWriteAdapter()->select()
                ...
                ->from(
                        array('e' => $this->getTable('catalog/product')), array_merge(array('entity_id', 'type_id'), $staticFields))
                ->join(
                        array('website' => $this->getTable('catalog/product_website')), $this->_getWriteAdapter()->quoteInto('website.product_id=e.entity_id AND website.website_id=?', $store->getWebsiteId()), array()
                )
                ->join(
                array('stock_status' => $this->getTable('cataloginventory/stock_status')), $this->_getWriteAdapter()->quoteInto('stock_status.product_id=e.entity_id AND stock_status.website_id=?', $store->getWebsiteId()), array('in_stock' => 'stock_status')
        );
        ...
        if (!$query->getIsProcessed()) {
            ...
            if ($searchType == Mage_CatalogSearch_Model_Fulltext::SEARCH_TYPE_LIKE
                ...
                foreach ($words as $word) {
                    ...
                    $like[] = '`s`.`name` LIKE :likew' . $likeI . ' OR `s`.`short_description` LIKE :likew' . $likeI;
                    $bind[':likew' . $likeI] = '%' . $word . '%';
                    $likeI++;
                    $clike .= $word;
                }
                //$clike1 = "replace(s.name,' ','') = '" . rtrim($clike) . "'";
                //$clike2 = "replace(s.name,' ','') = '" . rtrim($clike, "s") . "'";
                $clike1 = "replace(ccev.value,' ','') = '" . rtrim($clike) . "'";
                $clike2 = "replace(ccev.value,' ','') = '" . rtrim($clike, "s") . "'";
                if ($like) {
            ...
            $sql = sprintf("SELECT ccev.entity_id, ccei.value
FROM `catalog_category_entity_varchar` AS ccev, `catalog_category_entity_int` AS ccei
WHERE ccev.`entity_type_id` =3
AND ccei.entity_type_id =3
AND ccei.attribute_id = '32'
AND ccev.`attribute_id` =31
AND %s
AND ccev.entity_id = ccei.entity_id", $clike1);

            $results = $conn->fetchAll($sql);
            if (count($results) == 1) {
                $isd = $this->getParentCatId($conn, $results[0]['entity_id']);
                if ($isd == 1) {
                    $sql = "select value from catalog_category_entity_varchar where entity_type_id='3' and store_id='1' and attribute_id='47' and entity_id='" . $results[0]['entity_id'] . "'";
                    $results = $conn->fetchAll($sql);
                    if ($results[0]['value'] && count($results) == 1) {
                        $pat = Mage::getBaseURL() . $results[0]['value'];
                        Header("Location:$pat");
                        exit;
                    }
                }
...
AND ccev.`attribute_id` =31
AND %s
AND ccev.entity_id = ccei.entity_id", $clike2);
                $results = $conn->fetchAll($sql);
                if (count($results) == 1) {
                    $isd = $this->getParentCatId($conn, $results[0]['parent_id']);
                    if ($isd == 1) {
                        $sql = "select value from catalog_category_entity_varchar where entity_type_id='3'  and store_id='1' and attribute_id='47' and entity_id='" . $results[0]['entity_id'] . "'";
                        $results = $conn->fetchAll($sql);
                        if ($results[0]['value'] && count($results) == 1) {
            ...
            if (count($results) >= 1) {
                //$sql = sprintf("INSERT INTO `{$this->getTable('catalogsearch/result')}` (SELECT STRAIGHT_JOIN '%d', `s`.`entity_id`, '0' FROM `catalog_product_flat_1` AS `s` WHERE %s) ON DUPLICATE KEY UPDATE `relevance`=VALUES(`relevance`)", $query->getId(), $bcond);
                $sql = sprintf("INSERT INTO `{$this->getTable('catalogsearch/result')}` (SELECT STRAIGHT_JOIN '%d', `entity_id`, '0' FROM `catalog_product_entity_varchar` WHERE `entity_type_id` =4 AND `attribute_id` = '56' and %s) ON DUPLICATE KEY UPDATE `relevance`=VALUES(`relevance`)", $query->getId(), $bcond);
                foreach($bind as $key=>$val) {
                    $bind_str .=$key."~".$val;
                }
                //echo "<div style='display:none;' id='psearch'>" . $sql . " | " . $bind_str . "</div>";
            } else {
                $clike = "";
                foreach ($words as $word) {
                    $word = rtrim($word, "s");
                    $like[] = '`s`.`name` LIKE :likew' . $likeI;
                    $bind[':likew' . $likeI] = '%' . $word . '%';
                    $likeI++;
                ...
                if (count($results) >= 1) {
                    $sql = sprintf("INSERT INTO `{$this->getTable('catalogsearch/result')}` "
                            . "(SELECT STRAIGHT_JOIN '%d', `entity_id`, '0' FROM `catalog_product_entity_varchar` WHERE `entity_type_id` =4 AND `attribute_id` = '56' and (%s%s%s) )"
                            . " ON DUPLICATE KEY UPDATE `relevance`=VALUES(`relevance`)", $query->getId(), $fulltextCond, $separateCond, $clike, $query->getStoreId()
                    );
                } else {
                    $descSql = "SELECT entity_id FROM `catalog_product_entity_text` WHERE `entity_type_id` =4 AND `attribute_id` = '58' and ($clike)";
                    $sql = sprintf("INSERT INTO `{$this->getTable('catalogsearch/result')}` "
                            . "(SELECT STRAIGHT_JOIN '%d', `entity_id`, '0' FROM `catalog_product_entity_text` WHERE `entity_type_id` =4 AND `attribute_id` = '58' and (%s%s%s) )"
                            . " ON DUPLICATE KEY UPDATE `relevance`=VALUES(`relevance`)", $query->getId(), $fulltextCond, $separateCond, $clike, $query->getStoreId()
                    );
                }
            }
    ...
    public function getParentCatId($conn, $id = null, $ctr = null) {
        if ($sh == "")
            $sh = 1;
        if ($ctr == "")
            $ctr = 1;
            ...
            $ctr++;
        $sql = sprintf("SELECT parent_id,is_active,name FROM catalog_category_flat_store_1 as s WHERE s.entity_id='%s'", $id);
        $results = $conn->fetchAll($sql);
        if (count($results) > 0 && $results[0]['parent_id'] > 1) {
            if ($sh == 1)
                $sh = $results[0]['is_active'];
            else
                $sh = 0;
            if ($sh > 0) {
                $sh = $this->getParentCatId($conn, $results[0]['parent_id'], $ctr);
            } else {
                return $sh;
            }
        }
        ...
        foreach ($atributeTypes as $backendType => $attributeIds) {
            if ($attributeIds) {
                ...
                $selects[] = $this->_getWriteAdapter()->select()
                        ...
                        ->joinLeft(
                                array('t_store' => $tableName), $this->_getWriteAdapter()->quoteInto("t_default.entity_id=t_store.entity_id AND t_default.attribute_id=t_store.attribute_id AND t_store.store_id=?", $storeId), array('value' => 'IF(t_store.value_id > 0, t_store.value, t_default.value)'))
    ...
    protected function _getProductChildIds($productId, $typeId) {
        ...
        if ($relation && $relation->getTable() && $relation->getParentFieldName() && $relation->getChildFieldName()) {
            $select = $this->_getReadAdapter()->select()
                    ->from(
                            array('main' => $this->getTable($relation->getTable())), array($relation->getChildFieldName()))
