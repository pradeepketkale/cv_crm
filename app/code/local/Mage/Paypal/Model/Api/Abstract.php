...
abstract class Mage_Paypal_Model_Api_Abstract extends Varien_Object
{
   ...
   /**
    ...
    protected function _exportLineItems(array &$request, $i = 0)
    {
        ...
        if ($this->_lineItemTotalExportMap) {
            foreach ($this->_cart->getTotals() as $key => $total) {
                if (isset($this->_lineItemTotalExportMap[$key])) { // !empty($total)
					...
					if($key == 'tax') {
						$total = Mage::helper('directory')->currencyConvert($total,$baseCurrencyCode,$currency_code);
					}
		...
		if($publicKey == 'amount') {
			//Start: Added by suresh on 01-02-2012 for deduct discount
			if($i == 1 && $request['discount_amount'])
			{
				$value = ($value - $request['discount_amount']);
			}
			//End: Added by suresh on 01-02-2012 for deduct discount
			$value = Mage::helper('directory')->currencyConvert($value,$baseCurrencyCode,$currency_code);
		}
