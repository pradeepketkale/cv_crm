...
class Mage_Sales_Model_Convert_Parser_Exportorders
    extends Mage_Eav_Model_Convert_Parser_Abstract
{
    ...
    public function unparse()
    {
				...
				if($this->getVar('date_from') != "" && $this->getVar('date_to') != "" ) {
				
					$date_from = $this->getVar('date_from'). " 00:00:00";
					$date_to = $this->getVar('date_to'). " 23:59:59";
					$orders = Mage::getModel('sales/order')->getCollection()
									->addAttributeToSelect('increment_id')
									-> addAttributeToFilter ( 'updated_at' , array( "from" => $date_from, "to" => $date_to, "datetime" => true ))
									->load();
				} else {
          ...
          $row['website'] = $website->getCode();
					...
					if(method_exists($order, 'getShippingAddress')) {
					  ...
					  $row['shipping_street_2'] = $order->getShippingAddress()->getStreet(2);
						...
						$row['shipping_prefix'] = $order->getBillingAddress()->getData('prefix');
						$row['shipping_firstname'] = $order->getBillingAddress()->getData('firstname');
						$row['shipping_middlename'] = $order->getBillingAddress()->getData('middlename');
						$row['shipping_lastname'] = $order->getBillingAddress()->getData('lastname');
						$row['shipping_suffix'] = $order->getBillingAddress()->getData('suffix');
						$row['shipping_street_full'] = $order->getBillingAddress()->getStreet(1);
					  $row['shipping_street_2'] = $order->getBillingAddress()->getStreet(2);
						$row['shipping_city'] = $order->getBillingAddress()->getData('city');
						$row['shipping_region'] = $order->getBillingAddress()->getData('region');
						$row['shipping_country'] = $order->getBillingAddress()->getData('country_id');
						$row['shipping_postcode'] = $order->getBillingAddress()->getData('postcode');
						$row['shipping_telephone'] = $order->getBillingAddress()->getData('telephone');
						$row['shipping_company'] = $order->getBillingAddress()->getData('company');
						$row['shipping_fax'] = $order->getBillingAddress()->getData('fax');
					}
            ...
            $row['base_shipping_tax_refunded'] = $order->getData('base_shipping_tax_refunded');
						...
						foreach ($items as $itemId => $item)
						{
							/*
							 ...
							 $itemscount++;
							 */
							 $finalproductsorder .= $item->getSku() .":" . $item->getQtyToInvoice() . "|";
						}
