...
/*********************** ADDRESSES ***************************/
    ...
    public function getItemsCollectionByVendor($vendorId = null)
    {
    	if (is_null($this->_items)) {
    		$query = $this->_items = Mage::getResourceModel('sales/order_item_collection')
    ...
    public function getItemsCollectionByVendorMarketplace($vendorId = null)
    {
    	if (!is_null($this->_items)) {
    		$query = $this->_items = Mage::getResourceModel('sales/order_item_collection')
    		->addAttributeToFilter('udropship_vendor', $vendorId)
    ...
    /*
     * Flag is used for getting only vendor products at vendor login.and else part is for 
     * displaying products at user login per shipment wise.
     * 
     */
    public function getAllVisibleItemsByVendor($vendorId = null,$marketplaceFlag=false)
    {
    	$items = array();
        if($marketplaceFlag) {
        
            foreach ($this->getItemsCollectionByVendorMarketplace($vendorId) as $item) {
    		if (!$item->isDeleted() && !$item->getParentItemId()) {
    			$items[] =  $item;
    		}
            }
        } else{
    	foreach ($this->getItemsCollectionByVendor($vendorId) as $item) {
    		if (!$item->isDeleted() && !$item->getParentItemId()) {
    			$items[] =  $item;
    		}
    	}
