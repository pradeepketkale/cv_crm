...
class Mage_Catalog_Model_Convert_Adapter_Products extends Mage_Eav_Model_Convert_Adapter_Entity {
    ...
    public function saveRow(array $importData)
    {
       ...
       //Start:Added by suresh on 06-02-2012 for skip if vendor is empty or vendor not exists
       if (empty($importData['udropship_vendor'])){
            $message = Mage::helper('catalog')->__('Skip import row, required field "%s" not defined', 'udropship_vendor');
            Mage::throwException($message);
       }
       else {
       		$_vendor = Mage::getModel('udropship/vendor')
       				->getCollection()
       				->addFieldToFilter('vendor_name', array('in' => array($importData['udropship_vendor'])))
       				->getData();
       		if (empty($_vendor)){
            	$message = Mage::helper('catalog')->__('Skip import row, vendor "%s" not exists', $importData['udropship_vendor']);
            	Mage::throwException($message);
       		}		
       }
       

       //fix error if attribute set and config_attribute not match
            //By: saurabh
		if($importData['config_attributes']!='')
		{
            ...
            ->addFieldToFilter('attribute_set_name', $importData['attribute_set'])
            ->getFirstItem()
            ->getAttributeSetId();
            //Mage::log('hhhhhhhh========='.$attributeSetId);
            ...
            foreach($attributes as $_attribute){
                    ...
                    $allAttributes[]=$_attribute['code'];
            }

            $configAttributeCodes = $this->userCSVDataAsArray($importData['config_attributes']);
            foreach($configAttributeCodes as $value){

                if(!in_array($value,$allAttributes))
                {
                     $message = Mage::helper('catalog')->__('Skip import row, config_attributes not defined in attribute_set');
                    Mage::throwException($message);
                }
            }
		}
       ...
       //End:Added by suresh on 06-02-2012 for skip if vendor is empty or vendor not exists
		...
		foreach ($importData as $field => $value) {
			...
			if ($attribute->getFrontendInput() == 'multiselect') {
                ...
                //by saurabh for remove multiple attributes values
			...
			if($field == 'product_payment_methods')
			{
				$payment_array = explode(",",$importData['product_payment_methods']);
				$product->setData($field, $payment_array);
			}
			else {
		...
		*/
        ...
        foreach ($imageData as $file => $fields){
            try{
                $product->addImageToMediaGallery(Mage::getBaseDir('media') . DS . 'import' . $file, $fields,null, $exclude=false);
            }
            catch (Exception $e){}
        }
		//for multiple images
		...
		//$product->setQuantity(1);
        $sap=$importData['sap_sync'];
        $product->setSapSync($sap);
		$product->save();
		//Start:Added by suresh on 31-01-2012 for product tags
		if(isset($importData['product_tags'])){
        	$import_array = array();
        	if(strpos($importData['product_tags'], ',')) {
        		$import_array = explode(",", $importData['product_tags']);
        	}
        	else {
        		$import_array[] = $importData['product_tags'];
        	}
        	
        	foreach($import_array as $import_tag_val) {
        		$tagsvalue = Mage::getModel('tag/tag')->loadByName($import_tag_val);
       			$write_tags = Mage::getSingleton('core/resource')->getConnection('core_write');
       			if(!$tagsvalue->getId())
       			{
       				$write_tags->query("insert into tag (name, status) values ('".$import_tag_val."',1)");
       				$lastInsertId = $write_tags->lastInsertId();
       				$write_tags->query("insert into tag_properties (tag_id, store_id) values ('".$lastInsertId."',1)");
       				$write_tags->query("insert into tag_relation (tag_id, product_id, store_id, active, created_at) values ('".$lastInsertId."','".$product->getId()."', 1, 1, now())");
       				$write_tags->query("insert into tag_summary (tag_id, store_id, products) values ('".$lastInsertId."', 1, 1)");
       				$write_tags->query("insert into tag_summary (tag_id, store_id, products) values ('".$lastInsertId."', 0, 1)");
       			} else {
       				$readresult = $write_tags->query("select * from tag_relation where tag_id = ? and product_id = ? and store_id = 1 and active = 1", array($tagsvalue->getId(), $product->getId()));
       				if($readresult)
       				{
       					$readrows = $readresult->fetch(PDO::FETCH_ASSOC);
       					if(!$readrows)
       					{
       						$write_tags->query("insert into tag_relation (tag_id, product_id, store_id, active, created_at) values ('".$tagsvalue->getId()."','".$product->getId()."', 1, 1, now())");
       					}
       				} else {
       					$write_tags->query("insert into tag_relation (tag_id, product_id, store_id, active, created_at) values ('".$tagsvalue->getId()."','".$product->getId()."', 1, 1, now())");
       				}
       				
       				$readresult_summary = $write_tags->query("select * from tag_summary where tag_id = ? and store_id = 1", array($tagsvalue->getId()));
       				if($readresult_summary)
       				{
       					$readrows_summary = $readresult_summary->fetch(PDO::FETCH_ASSOC);
       					if(!$readrows_summary)
       					{
       						$write_tags->query("insert into tag_summary (tag_id, store_id, products) values ('".$tagsvalue->getId()."', '1', 1)");
       						$write_tags->query("insert into tag_summary (tag_id, store_id, products) values ('".$tagsvalue->getId()."', '0', 1)");
       					}
       					else {
       						$write_tags->query("update tag_summary set products = (tag_summary.products + 1) where tag_id = '".$tagsvalue->getId()."' and  store_id = 1");
       						$write_tags->query("update tag_summary set products = (tag_summary.products + 1) where tag_id = '".$tagsvalue->getId()."' and  store_id = 0");
       					}
       				}
       				else {
       					$write_tags->query("insert into tag_summary (tag_id, store_id, products) values ('".$tagsvalue->getId()."', 1, 1)");
        ...
        //End:Added by suresh on 31-01-2012 for product tags
        return true;
    }
	...
	public function updateSourceAndReturnId($attribute_code,$newOption)
	{
		...
		try{
			//added condition if attribute value is empty then not insert into table
			if(trim($newOption)) {
	...
	protected function _addCategories($importData, $store)
    {
		...
		//Commented by suresh jan 17 2012
		//$rootId = $store->getRootCategoryId();
		//End commented by suresh
		
		//Static root Id given by suresh jan 17 2012
        $rootId = 2;
        //End Static root Id given by suresh jan 17 2012
        ...
        foreach (explode(',', $categories) as $categoryPathStr){
			...
			//Commented by suresh jan 17 2012
			//$catIds			=	array();
			//End Commented by suresh jan 17 2012
