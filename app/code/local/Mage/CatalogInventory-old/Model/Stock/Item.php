...
class Mage_CatalogInventory_Model_Stock_Item extends Mage_Core_Model_Abstract {
    ...
    public function subtractQty($qty) {
        //$orderId = Mage::helper('checkout/cart')->getOrder()->getId();
        $quoteId = Mage::getSingleton('checkout/session')->getQuoteId();
        $product_id = $this->getProductId();
        $lastQty = $this->getStockQty();
        $newQty = $lastQty - $qty;
        $sql = "insert into cataloginventory_stock_item_audit(product_id,lastQty,changetype,changedQty,newQty,quoteId) values('$product_id','$lastQty','subtractQty','-$qty','$newQty','$quoteId');";
    ...
    public function addQty($qty) {
		...
		if (Mage::app()->getStore()->isAdmin())
        	$curUser = Mage::getSingleton('admin/session')->getUser()->getusername();

        $orderId = Mage::app()->getRequest()->getParam('order_id');
        $quote = Mage::getModel('sales/order')->load($orderId)->getData();
        $quoteId = $quote[quote_id];
        $product_id = $this->getProductId();
        $lastQty = $this->getStockQty();
        $newQty = $lastQty + $qty;
        $sql = "insert into cataloginventory_stock_item_audit(product_id,lastQty,changetype,changedQty,newQty,userId,quoteId) values('$product_id','$lastQty','addQty','$qty','$newQty','$curUser','$quoteId')";
        $write = Mage::getSingleton('core/resource')->getConnection('core_write');
        $write->query($sql);
        $this->setQty($this->getQty() + $qty);
        return $this;
