...
class Mage_Adminhtml_Model_Sales_Order_Create extends Varien_Object {
    ...
    public function initFromOrder(Mage_Sales_Model_Order $order) {
        ...
        foreach ($order->getItemsCollection(
                array_keys(Mage::getConfig()->getNode('adminhtml/sales/order/create/available_product_types')->asArray()), true
        ) as $orderItem) {
        ...
        Mage::helper('core')->copyFieldset(
                'sales_copy_order', 'to_edit', $order, $this->getQuote()
        );
    ...
    protected function _initBillingAddressFromOrder(Mage_Sales_Model_Order $order) {
        ...
        Mage::helper('core')->copyFieldset(
                'sales_copy_order_billing_address', 'to_order', $order->getBillingAddress(), $this->getQuote()->getBillingAddress()
        );
    }
    ...
    protected function _initShippingAddressFromOrder(Mage_Sales_Model_Order $order) {
        ...
        Mage::helper('core')->copyFieldset(
                'sales_copy_order_shipping_address', 'to_order', $order->getShippingAddress(), $this->getQuote()->getShippingAddress()
        );
    }
    ...
    public function createOrder() {
        try {
            ...
            //echo "<li> session".$this->getSession()->getOrder()->getId();

            $flag = true;
            $curItemQty = array();
            $qId = $this->getSession()->getOrder()->getQuoteId();
            $sql = "SELECT product_id, qty from sales_flat_quote_item where quote_id='$qId'";
            $conn = Mage::getSingleton('core/resource')->getConnection('core_read');
            $results = $conn->fetchAll($sql);
            if ($results) {
                foreach ($results as $key => $value) {
                    $productQty = Mage::getModel('cataloginventory/stock_item')->loadByProduct($results[$key]['product_id'])->getQty();
                    //echo "<li>" . $results[$key]['product_id'] . "---" . $results[$key]['qty'] . "---" . $productQty;
                    $curItemQty[$results[$key]['product_id']] = $results[$key]['qty'] + $productQty;
                }
            }

            $oldItemQty = array();
            $sql = "SELECT product_id, qty from sales_flat_quote_item where quote_id='" . $quote->getId() . "'";
            $results2 = $conn->fetchAll($sql);
            if ($results2) {
                foreach ($results2 as $key => $value) {
                    if ($curItemQty[$results2[$key]['product_id']] && $curItemQty[$results2[$key]['product_id']] != "") {
                        if ($curItemQty[$results2[$key]['product_id']] < $results2[$key]['qty']) {
                            $product = Mage::getModel('catalog/product')->load($results2[$key]['product_id']);
                            $productType = $product->getTypeId();
                            $productName = $product->getName();
                            $productSKU = $product->getSku();
                            if ($productType == "simple") {
                                $flag = false;
                                $pName .="<br/>" . $productName . "- SKU:" . $productSKU . "- InStock" . $curItemQty[$results2[$key]['product_id']];
                            }
                        }
                    } else {
                        $product = Mage::getModel('catalog/product')->load($results2[$key]['product_id']);
                        ...
                        $productName = $product->getName();
                        $productSKU = $product->getSku();
                        $stock_count = (int) Mage::getModel('cataloginventory/stock_item')->loadByProduct($results2[$key]['product_id'])->getQty();
                        if ($productType == "simple" && $stock_count < $results2[$key]['qty']) {
                            $flag = false;
                            $pName .="<br/>Product ID:".$results2[$key]['product_id']." Name:" . $productName . " - SKU:" . $productSKU . "- InStock" . $stock_count;
                        }
                    }
                }
            }
            ...
            if ($flag == false) {
                $errmsg = 'The requested quantity is not available for' . $pName . '</br>Kindly edit the quantity and try again.';
                Mage::throwException(Mage::helper('cataloginventory')->__($errmsg));
                //    exit;
            } else {
