...
class Mage_Adminhtml_Block_Sales_Order_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if ($this->_isAllowedAction('edit') && $order->canEdit()  && strtolower($order->getStatus())!="complete" && strtolower($order->getStatus())!="processing") {
		...
		//Below line commented by Dileswar for removing the Undo cancel for cash in advance on dated 20-12-2012
		
	//if ($this->_isAllowedAction('undo') && $this->getOrder()->canUndo() && $this->getOrder()->getPayment()->getMethodInstance()->getTitle() != 'Cash In Advance') {
		//if ($this->_isAllowedAction('undo') && $this->getOrder()->canUndo()) {
			if ($this->getOrder()->canUndo()) {
            $message = Mage::helper('sales')->__('Are you sure you want to undo this order?');
            $this->_addButton('order_undo', array(
                'label'     => Mage::helper('sales')->__('Undo Cancel'),
                'onclick'   => 'deleteConfirm(\''.$message.'\', \'' . $this->getUndoUrl() . '\')',
            )); 
       }
     ...
     */
	 ...
	 public function getUndoUrl()
    {
        return $this->getUrl('*/*/undo');
    } 
