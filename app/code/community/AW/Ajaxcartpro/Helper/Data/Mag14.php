...
class AW_Ajaxcartpro_Helper_Data_Mag14 extends AW_Ajaxcartpro_Helper_Data {
    ...
    public function renderTopCartLinkTitle() {
        ...
        $grand_total_val = Mage::helper('checkout')->formatPrice(Mage::getSingleton('checkout/cart')->getQuote()->getGrandTotal());
        
        /*Start...
        * Craftsvilla Comment
        * commented and aaded new code for change header cart display
        * Added by suresh 29-05-2012
        */
        if( $count == 1 ) {
        	//$text = Mage::helper('checkout')->__('My Cart (%s item)', $count);
            $text = Mage::helper('checkout')->__('<a href="'.Mage::getBaseUrl().'checkout/cart/"><span class="icon icon-cart"></span><strong>Cart</strong> {%s} Item ', $count).$grand_total_val.'</a>';
        } elseif( $count > 0 ) {
            //$text = Mage::helper('checkout')->__('My Cart (%s items)', $count);
            $text = Mage::helper('checkout')->__('<a href="'.Mage::getBaseUrl().'checkout/cart/"><span class="icon icon-cart"></span><strong>Cart</strong> {%s} Items ', $count).$grand_total_val.'</a>';
        } else {
