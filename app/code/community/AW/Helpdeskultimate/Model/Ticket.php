...
class AW_Helpdeskultimate_Model_Ticket extends AW_Core_Model_Abstract
{
    ...
    public function generateUid()
    {
        do {
            ...
            $a_z_count = strlen($a_z);
            for($i=0; $i<3; $i++){
                $int = rand(0, $a_z_count-1);
                $letters .= $a_z[$int];
            }
    ...
    public function _afterSave()
    {
        ...
        if (
            ...
            Mage::getModel('awcore/logger')
                    ->setData(array(
                                   'title' => "Ticket #{$this->getUid()} status changed to '{$this->getStatusText()}' by '{$this->getDepartment()->getName()}'",
    ...
    {
        return Mage::getSingleton('adminhtml/url')->getUrl('helpdeskultimate_admin/ticket/edit', array('id' => $this->getId()));
    }
    ...
    {
        return Mage::getModel('core/url')->setStore($this->getStoreId())->getUrl('helpdeskultimate/customer/view', array('id' => $this->getId()));
    }
    ...
    public function getOldDepartment()
    {
        if (!$this->getData('old_department')) {
            $this->setData('old_department', Mage::getModel('helpdeskultimate/department')->load($this->getOrigData('department_id')));
        }
    ...
    public function getAccessUrl($forceExternalUrl = null)
    {
        if ($forceExternalUrl === null)
            $forceExternalUrl = Mage::getStoreConfig(AW_Helpdeskultimate_Helper_Config::XML_PATH_ALLOWEXTERNAL, $this->getStoreId());
        if ($this->getCustomerId() && !$forceExternalUrl) {
            return $this->getCustomerUrl();
        }
        return Mage::getModel('core/url')
                ->setStore($this->getStoreId())
                ->getUrl('helpdeskultimate/customer/viewext/',
                         array(
    ...
    public function getCustomerUrlHtml()
    {
        return
                Mage::helper('helpdeskultimate')->__(
                    'You can view the ticket and reply from web interface from %s here %s.', '<a href="' . $this->getAccessUrl() . '">', '</a>');
