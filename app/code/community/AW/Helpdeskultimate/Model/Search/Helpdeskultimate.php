...
class AW_Helpdeskultimate_Model_Search_Helpdeskultimate extends Varien_Object
{
    ...
    public function load()
    {
        ...
        $table_messages = Mage::getResourceModel('helpdeskultimate/message')->getTable('helpdeskultimate/message');
        $collection->getSelect()
                ->columns('GROUP_CONCAT(m.content SEPARATOR " ") AS message_content')
                ->joinLeft(array('m' => $table_messages),
        ...
        foreach ($collection as $ticket) {
            ...
            $arr[] = array(
                ...
                'name' => Mage::helper('helpdeskultimate')->__('Ticket #%s', $ticket->getUid()) . " {$ticket->getTitle()}",
                'description' => $ticket->getCustomerName() . ' ' . $ticket->getCustomerEmail(),
                'form_panel_title' => Mage::helper('helpdeskultimate')->__('Ticket #%s (%s)', $ticket->getUid(), $ticket->getCustomerName() . ' ' . $ticket->getCustomerEmail()) . " {$ticket->getTitle()}",
                'url' => Mage::getSingleton('adminhtml/url')->getUrl('helpdeskultimate_admin/ticket/edit', array('id' => $ticket->getId())),
            );
        }
