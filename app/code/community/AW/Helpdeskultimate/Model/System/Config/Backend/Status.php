...
class AW_Helpdeskultimate_Model_System_Config_Backend_Status extends Mage_Core_Model_Config_Data
{
    ...
    protected function _beforeSave()
    {
        ...
        if (count($deletedLabels)) {
            /*
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('helpdeskultimate')->__("Status(es) \"%s\" was successfully deleted", implode("\", \"", $deletedLabels)));
            */
        }
        if (count($undeletedLabels)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('helpdeskultimate')->__("Status(es) \"%s\" cannot be deleted because used in tickets", implode("\", \"", $undeletedLabels)));
        }
