...
class AW_Helpdeskultimate_RejectingemailsController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->_initAction();
        if (!$this->getRequest()->getParam('fswe') || !$this->_getHelper()->getRejectedFormData($this->getRequest()->getParam('id'))) {
            $_formData = Mage::getModel('helpdeskultimate/rpattern')->load($this->getRequest()->getParam('id'));
        ...
        $__title = is_null($_formData->getId()) ? $this->__('Pattern') : $_formData->getName();
        $this->_title($this->__('Patterns List'))
             ->_title($__title)
        ;
        $this->renderLayout();
        ...
        if ($_request->getParam('name')) {
            ...
            if ($_request->getParam('scope')) {
                ...
                if ($_request->getParam('pattern')) {
                    ...
                    if (preg_match("/\/.*\/[imsxADSUX]*$/", $_data['pattern']) && !(@preg_match($_data['pattern'], '') === false)) {
                        if (!preg_match("/\/.*\/.*[m].*$/", $_data['pattern'])) {
                            $_data['pattern'] .= 'm';
                        }
                    } else {
                        $this->_getSession()->addError($this->__('Wrong pattern syntax. Allowed format: /.*/imsxADSUX'));
                    }
                } else {
