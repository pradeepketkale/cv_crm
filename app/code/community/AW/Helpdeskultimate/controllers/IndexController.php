...
class AW_Helpdeskultimate_IndexController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        if (!count($deparpments)) {
            Mage::getSingleton('adminhtml/session')->addError($this->__('You have no departments configured yet. Please configure at least one active department <a href="%s">here</a>', Mage::helper('adminhtml')->getUrl('helpdeskultimate_admin/departments/index')));
        }
        ...
        if (!is_array($ids)) {
            ...
            try {
                ...
                foreach ($ids as $id) {
                    ...
                    if (Mage::getSingleton('helpdeskultimate/status')->isAllowToSet($ticket->getStoreId(), $this->getRequest()->getParam('status'))) {
                        $ticket
                ...
                if (count($dissAllowed)) {
                    $this->_getSession()->addError(
                        $this->__(
                            ...
                            implode('", "', $dissAllowed),
                            Mage::getSingleton('helpdeskultimate/status')->getStatusLabel($this->getRequest()->getParam('status'))
                        ));
                }
            } catch (Exception $e) {
            ...
            try {
                foreach ($ids as $id) {
                    $ticket = Mage::getSingleton('helpdeskultimate/ticket')
                            ->load($id);
                    $lockId = ($this->getRequest()->getParam('locked'))?Mage::getSingleton('admin/session')->getUser()->getId():0;
