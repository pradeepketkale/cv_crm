...
class AW_Helpdeskultimate_TicketController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('helpdeskultimate/index');
    }
    ...
    public function editAction()
    {
        ...
        $__title = is_null($ticket->getId())?$this->__('Create New Ticket'):'#' . $ticket->getUid();
        $this->_title( $__title );
    ...
    public function saveTicketAction()
    {
        ...
        try {
            if ($data = $this->getRequest()->getPost()) {
                ...
                if (!is_null($uploader)) {
                    $ticket->setFilename($_FILES['filename']['name']);
                }
                ...
                /* Assign order inc id*/
                $ticket->setOrderIncrementalId(preg_replace("/[^0-9a-z\-]+/sui", "", urldecode(@$data['order_incremental_id'])));
                ...
                $ticket
                        ...
                        ->setStoreId((isset($data['store_id'])) ? $data['store_id'] : 1)
                        ->setTitle($data['title'])
                ...
                if (!is_null($uploader)) {
                    $fileName = Mage::helper('helpdeskultimate')->getEncodedFileName($ticket->getData('filename'));
                ...
                if (($ticket->getCustomerEmail()) && (isset($data['email']) && $data['email'])) {
                    Mage::helper('helpdeskultimate/notify')->ticketNew($ticket);
                }
    ...
    public function saveAction()
    {
        try {
            if ($data = $this->getRequest()->getPost()) {
                ...
                if ($ticket->isReadOnly()) {
                    throw(new Exception(
                        $this->__("Ticket has been locked by %s at %s", $ticket->getLockedUser()->getName(), $ticket->getLockedAt())
                    ));
                }
                if ($data['content'] && $ticket->getStatus() == AW_Helpdeskultimate_Model_Status::STATUS_OPEN && $data['status_id'] == $ticket->getStatus())
                    $ticket->setStatus(AW_Helpdeskultimate_Model_Status::STATUS_WAITING);
                else
                    $ticket->setStatus($data['status_id']);
                ...
                if (
                    ($department->getId() != $ticket->getDepartmentId())
                ) {
                    // Department changed. Notify new department about it
                    $ticket
                            ->setDepartmentId(intval($department->getId()));
                ...
                if ($ticket->getOrderIncrementalId()) {
                    // Try to assign order
                    if ($orderId = Mage::getModel('sales/order')->loadByIncrementId($ticket->getOrderIncrementalId())->getId()) {
                        $ticket->setOrderId($orderId);
                    }
                }
                ...
                if (trim($data['content'])) {
                    ...
                    $message
                            ...
                            ->setDepartmentId($department->getId())
                            ->setAuthorName(
                                $ticket->getOldDepartment()->getName()
                            )
                            ->save();
    ...
    public function usersuggestAction()
    {
        ...
        $options = array();
        foreach ($unregistered as $unreg_user) {
            if (!$unreg_user->getEmail()) {
                continue;
            }
            $flag = true;
            foreach ($users->getCollection() as $reg_user) {
                if ($reg_user->getEmail() == $unreg_user->getEmail()) {
                    $flag = false;
                    break;
                }
            }
            if ($flag) {
                if ($unreg_user->getEmail() == $letters) {
                    $match100 = 1;
                }
                $options[] = array(
                    'label' => "{$unreg_user->getFirstname()} {$unreg_user->getLastname()} <{$unreg_user->getEmail()}> [Unregistered]",
                    'value' => "{$unreg_user->getFirstname()} {$unreg_user->getLastname()} {$unreg_user->getEmail()}",
                    'is_belong_to_order' => Mage::helper('helpdeskultimate')->isOrderBelongToCustomer($orderId, null, $unreg_user->getEmail())
                );
            }
        }
        ...
        foreach ($users->getCollection() as $user) {
            ...
            $options[] = array(
                   ...
                   'value' => $user->getId(),
                   'is_belong_to_order' => Mage::helper('helpdeskultimate')->isOrderBelongToCustomer($orderId, $user->getId(), $user->getEmail())
            );
        }
        ...
        if (!$match100) {
            ...
            if ($email) {
                $options[] = array(
                    ...
                    'is_belong_to_order' => Mage::helper('helpdeskultimate')->isOrderBelongToCustomer($orderId, null, $email)
                );
            }
    ...
    {
        $orderId = preg_replace("/[^0-9a-z\-]+/sui", "", urldecode($this->getRequest()->getParam('order_id')));
        ...
        if ($orderId != '') {
            ...
            foreach ($orderList as $order) {
                ...
                $likeOrder[$i]['is_belong_to_customer'] = Mage::helper('helpdeskultimate')->isOrderBelongToCustomer($order->getId(), $customerId, $customerEmail);
                $i++;
            }
    ...
    public function removemessageAction()
    {
        ...
        return $this->_redirect('helpdeskultimate_admin/ticket/edit', array('id' => $this->getRequest()->getParam('id')));
    }
    ...
    public function savemessagebodyAction()
    {
        ...
        $ticket = $message->getTicket();
        if ($ticket->isReadOnly()) {
            $result['text'] = $this->__('Ticket has been locked by %s at %s', $ticket->getLockedUser()->getName(), $ticket->getLockedAt());
        ...
        if ($message->getData()) {
            ...
            $message->save();
            $result['s'] = true;
            $result['text'] = Mage::getModel('helpdeskultimate/data_parser')->setText($message->getParsedContent())->prepareToDisplay()->getText();
            $result['quotetext'] = Mage::getModel('helpdeskultimate/data_parser')->setText($message->getParsedContent())->convertToQuoteAsHtml()->getText();
            $result['dbtext'] = $message->getParsedContent();
            if ($message->isDepartmentReply()) {
    ...
    private function _createUploader()
    {
        if (isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
            ...
            if ($_FILES['filename']['size'] > ($this->_helper('config')->getAttachmentsMaxUploadFileSize() * self::SIZE_MEGABYTE) || $_FILES['filename']['error'] === UPLOAD_ERR_INI_SIZE) {
