...
class AW_Helpdeskultimate_CustomerController extends Mage_Core_Controller_Front_Action
{
    ...
    public function viewextAction()
    {
        ...
        if ($ticket = Mage::getModel('helpdeskultimate/ticket')->loadExternal(urldecode($uid), $key)) {
            if ($ticket->getCustomerId() && ($ticket->getCustomerId() == $this->_getCustomerSession()->getCustomerId())) {
    ...
    public function newAction()
    {
        ...
        try {
            ...
            if (isset($postData['order_id']) && intval($postData['order_id']) > 0 && $this->_helper('config')->isAllowedOrdersLinking()) {
            ...
            if (!is_null($uploader)) {
                ...
                @mkdir($path);
                $__filename = Mage::helper('helpdeskultimate')->getEncodedFileName($Proto->getData('filename'));
                $uploader->save($path, $__filename);
            }
    ...
    public function postReplyAction()
    {
        $uid = $this->getRequest()->getParam('uid');
        $key = $this->getRequest()->getParam('key');
        if ($uid && $key && ($ticket = Mage::getModel('helpdeskultimate/ticket')->loadExternal(urldecode($uid), $key))) {
            if (!$this->_helper('config')->isAllowedExternalView()) {
                $this->_redirect('helpdeskultimate/customer');
            }
        ...
        if (isset($customer)) {
            if ($ticket->getData('customer_id')) {
                ...
                $this->_redirect('*/*/viewext', array('uid' => $this->getRequest()->getParam('uid'),
                                                      'key' => $this->getRequest()->getParam('key')));
            }
        } else {
            $this->_redirectReferer();
    ...
    private function _createUploader()
    {
        if (isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
            ...
            if ($_FILES['filename']['size'] > ($this->_helper('config')->getAttachmentsMaxUploadFileSize() * self::SIZE_MEGABYTE) || $_FILES['filename']['error'] === UPLOAD_ERR_INI_SIZE) {
                throw(
                new Exception(
                    $this->__("Uploaded file is too large")
                )
                );
            }
            /* Starting upload */
