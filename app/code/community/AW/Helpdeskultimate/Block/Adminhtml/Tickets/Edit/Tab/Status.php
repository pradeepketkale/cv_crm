    ...
    public function getChangerName($entry)
    {
        try {
            if ($entry->getData('custom_field_4')) {
                // Changed by customer
                return '[C] ' . Mage::getModel('helpdeskultimate/ticket')->load($entry->getData('custom_field_1'))->getCustomer()->getName();
            } else {
                return '[D] ' . Mage::getModel('helpdeskultimate/department')->load($entry->getData('custom_field_3'))->getName();
