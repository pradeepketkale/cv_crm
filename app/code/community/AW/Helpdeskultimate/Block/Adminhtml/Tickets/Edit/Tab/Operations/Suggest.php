    ...
    public function getHtml()
    {
        ...
        if (count($likeOrder)) {
            foreach($likeOrder as $item){
                $__cssClass = $item['is_belong_to_customer'] ? 'belong-to-customer' : 'not-belong-to-customer';
                $html .= "<li id='" . $item['value'] . "' class='" . $__cssClass . "'>" . $item['label'] . "</li>\n";
            }
        }
        else {
        ...
        $html .= "</ul>";
        $count = $this->getCount();
        $html .= "<div class='total-count'>" . $this->helper('helpdeskultimate')->__(self::TOTAL_COUNT, $count) . "</div>\n";
        return $html;
    }
}