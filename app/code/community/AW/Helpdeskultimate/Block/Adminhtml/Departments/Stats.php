...
class AW_Helpdeskultimate_Block_Adminhtml_Departments_Stats extends Mage_Adminhtml_Block_Template
{
    ...
    {
        $deps = Mage::getModel('helpdeskultimate/department')->getCollection()->getData();
        ...
        foreach ($deps as $dep) {
            ...
            $select = $db->select()
                    ->from(array('ticket' => $resource->getTableName('helpdeskultimate/ticket')), array('status', 'count(*) as total'))
                    ->group('status')
            ...
            if ($to)
                $select->where('created_time <= ?', date('Y-m-d H:i:s', strtotime($to)));
