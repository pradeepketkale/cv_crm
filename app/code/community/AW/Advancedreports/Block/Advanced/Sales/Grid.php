...
class AW_Advancedreports_Block_Advanced_Sales_Grid extends AW_Advancedreports_Block_Advanced_Grid
{
	...
	{
		$invoiceTable = $this->getTable('sales_flat_invoice');
		$invoiceItemTable = $this->getTable('sales_flat_invoice_item');
		$this->getCollection()->getSelect()
			->joinLeft( array('invoiceitem'=>$invoiceItemTable), "(invoiceitem.order_item_id = item.item_id)")
			->joinLeft( array('invoice'=>$invoiceTable), "(invoice.entity_id = invoiceitem.parent_id)" , array('invoice_increment_id' => 'increment_id'))
			->order('e.created_at DESC')
			;
		return $this;
	}
...
//		echo '</pre>';		
    ...
    {	
		...
		$this->addColumn('invoice_increment_id', array(
            'header' => Mage::helper('advancedreports')->__('Invoice #'),
