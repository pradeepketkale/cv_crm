 ...
 */ 
class AW_Productrelater_Block_Rewrite extends Mage_Catalog_Block_Product_List_Related {
    protected function _beforeToHtml() {
        ...
        switch(Mage::helper('productrelater')->getMagentoVersionCode()) {
            case AW_Productrelater_Helper_Data::MAGENTO_VERSION_CE_1_4:
                $_template = 'productrelater/native_related14.phtml';
                break;
            case AW_Productrelater_Helper_Data::MAGENTO_VERSION_CE_1_3:
                $_template = 'productrelater/native_related13.phtml';
                break;
        }
    ...
    public function _prepareData()	{
        ...
        //replace standard related products
        if(!$this->getReplaceoriginalrp()) return parent::_prepareData();

        if(!(int)$this->getItemscount()||(int)$this->getItemscount()<1)     $this->setItemscount(3);
        if(($this->getSource()!=='1'&&$this->getSource()!=='2')
            ||!$this->getProduct())                                                 $this->setSource(1);
        if((int)$this->getSelectproducts()<1||(int)$this->getSelectproducts()>3)    $this->setSelectproducts(2);
        if(!$this->getProduct()&&$this->getSelectproducts()==3)                     $this->setSelectproducts(2);
        if((int)$this->getFetchinstock()<0||(int)$this->getFetchinstock()>1||!$this->getFetchinstock())     $this->setFetchinstock(0);
