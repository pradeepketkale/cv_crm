...
class Gharpay_Model_Gharpay extends Mage_Core_Model_Abstract 
{
	...
	public static function viewOrderStatus($gharpayOrderID){
		...
		if($orderStatus == 'Delivered')
		{
			...
			if($salesModel->getStatus() == 'canceled')
			{
				...
				mail('bhavik@craftsvilla.com', 'Secont attempt to order canceled to Processing', $message, $headers);
			}
			else{
		...
		if($orderStatus == 'Failed' || $orderStatus == 'Cancelled by Customer' || $orderStatus == 'Invalid')
		{
			...
			if($salesModel->getStatus() == 'processing')
			{
				...
				$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
				$headers .= 'From: raf@craftsvilla.com' . "\r\n";
				mail('bhavik@craftsvilla.com', 'Secont attempt to order Processing to canceled', $message, $headers);
			}else{
				$salesModel->cancel();
				$salesModel->save();
			}
	...
	public static function getAllPincodes(){
		$url = 'http://services.gharpay.in/rest/GharpayService/getAllPincodes';
		$ch = curl_init($url) ;
		curl_setopt($ch,CURLOPT_HEADER,true);
		curl_setopt($ch,CURLOPT_POST, false);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1 );
	...
	public static function getCityList(){
		$url = 'http://services.gharpay.in/rest/GharpayService/getCityList';
		$ch = curl_init($url) ;
		curl_setopt($ch,CURLOPT_HEADER,true);
		curl_setopt($ch,CURLOPT_POST, false);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1 );
