...
class Payu_PayuCheckout_Model_Shared extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getFormFields()
    {
	   ...
	   // $txnid = $orderId+370000; 
		...
		$coFields['furl']         =  Mage::getBaseUrl().'payucheckout/shared/failure/id/'.$this->getOrder()->getRealOrderId();
	   ...
	   $salt=Mage::getStoreConfig('payment/payucheckout_shared/salt');
	    if(isset($response['status']))
		{
		  ...
		  // $orderid=$txnid-370000;
		   ...
		   {
		       $order->loadByIncrementId($orderid);
		            //Below both lines commnted by dileswar to reload the page to pay u failure on dated 30-01-2014 1.CANCELLED & cancel(),inventory updated line also
