...
class OnePica_AvaTax_Model_Observer extends Mage_Core_Model_Abstract
{
    ...
    public function adminSystemConfigChangedSectionTax($schedule) {
		...
		if(strpos(Mage::getStoreConfig('tax/avatax/url', $storeId), 'development.avalara.net') !== false) {
			$warnings[] = 'You are using the AvaTax development connection URL. If you are receiving errors about authentication, please ensure that you have a development account.';
		}
    	if(Mage::getStoreConfig('tax/avatax/action', $storeId) == OnePica_AvaTax_Model_Config::ACTION_DISABLE) {
    		$warnings[] = 'All AvaTax services are disabled';
    	}
    	if(Mage::getStoreConfig('tax/avatax/action', $storeId) == OnePica_AvaTax_Model_Config::ACTION_CALC) {
    		$warnings[] = 'Orders will not be sent to the AvaTax system';
    	}
    	if(Mage::getStoreConfig('tax/avatax/action', $storeId) == OnePica_AvaTax_Model_Config::ACTION_CALC_SUBMIT) {
    		$warnings[] = 'Orders will be sent but never committed to the AvaTax system';
    	}
    	...
    	if(!Mage::getStoreConfig('tax/avatax/account', $storeId)) {
    		$errors[] = 'You must enter an account number';
    	}
    	...
    	if(!is_numeric(Mage::getStoreConfig('tax/avatax/log_lifetime'))) {
    		$errors[] = 'You must enter the number of days to keep log entries';
    	}
    	if(!Mage::getStoreConfig('tax/avatax/company_code', $storeId)) {
    		$errors[] = 'You must enter a company code';
    	}
    	if(!Mage::getStoreConfig('tax/avatax/shipping_sku', $storeId)) {
    		$errors[] = 'You must enter a shipping sku';
    	}
    	if(!Mage::getStoreConfig('tax/avatax/adjustment_positive_sku', $storeId)) {
    		$errors[] = 'You must enter an adjustment refund sku';
    	}
    	if(!Mage::getStoreConfig('tax/avatax/adjustment_negative_sku', $storeId)) {
    		$errors[] = 'You must enter an adjustment fee sku';
    	}
    	
    	if(!class_exists('SoapClient')) {
    		$errors[] = 'The PHP class SoapClient is missing. It must be enabled to use this extension. See <a href="http://www.php.net/manual/en/book.soap.php" target="_blank">http://www.php.net/manual/en/book.soap.php</a> for details.';
    	}
    	
    	if(!function_exists('openssl_sign') && count($errors)) {
    		$key = array_search('SSL support is not available in this build', $errors);
    		if(isset($errors[$key])) unset($errors[$key]);
    		$errors[] = 'SSL must be enabled in PHP to use this extension. Typically, OpenSSL is used but it is not enabled on your server. This may not be a problem if you have some other form of SSL in place. For more information about OpenSSL, see <a href="http://www.php.net/manual/en/book.openssl.php" target="_blank">http://www.php.net/manual/en/book.openssl.php</a>.';
    	}
    	...
    	if(count($errors) == 1) {
			...
			$session->addError('Please fix the following issues:<br /> - ' . implode('<br /> - ', $errors));
    	}
    	
    	if(count($warnings) == 1) {
			$session->addWarning(implode('', $warnings));
    	} elseif(count($warnings)) {
			$session->addWarning('Please be aware of the following warnings:<br /> - ' . implode('<br /> - ', $warnings));
    	}
    }
    ...
    public function cleanLog($schedule) { 
		$mintime = strtotime('-' . floatval(Mage::getStoreConfig('tax/avatax/log_lifetime')) . ' days');
		$collection = Mage::getModel('avatax_records/log')->getCollection()
						->addFieldToFilter('created_at', array('lt'=>gmdate('Y-m-d H:i:s', $mintime)))
						->load();
