...
class RicoNeitzel_PaymentFilter_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	{
        $productPaymentMethods = array();
// Commented For stop checkmo query on dated 28-03-2015		
		...
		*/
		// Added By Amit Pitre On (4 Apr 2012) as check payment methods disabled on vendor level and admin level.
		$productUdropshipVendor = '';
		$_helpv = Mage::Helper('udropship');
		if($product->getUdropshipVendor())
			$productUdropshipVendor = $product->getUdropshipVendor();
		else {
			$productUdropshipVendor = $_helpv->getLocalVendorId(Mage::app()->getStore()->getStoreId());
		}
		if($productUdropshipVendor) {
			$vendorIfo = $_helpv->getVendor($productUdropshipVendor);
			$vendorPaymentMethods = $vendorIfo->getVendorPaymentMethods();
			$vendorAdminPaymentMethods = $vendorIfo->getVendoradminPaymentMethods();
			if($vendorPaymentMethods) {
				foreach ($vendorPaymentMethods as $method)
				{
					if (! in_array($method, $productPaymentMethods)) $productPaymentMethods[] = $method;
				}
			}
			if($vendorAdminPaymentMethods) {
	...
	public function getAllowedPaymentMethodsForCurrentGroup()
	{
            ...
            //code to remove COD from vendor other then sanvi
            /*$items=Mage::getSingleton('checkout/cart')->getQuote()->getAllItems();
            foreach($items as $item)
                    {
                        $prodId=$item->getProductId();
                        $vendorid=Mage::getModel('catalog/product')->load($prodId)->getUdropshipVendor();
                        $vendorId=explode(",",$vendorid);
                        
                        $array=$this->getCurrentCustomerGroup()->getAllowedPaymentMethods();
                        if(in_array('1',$vendorId))
                        {
                            $arr1=$this->getCurrentCustomerGroup()->getAllowedPaymentMethods();
                        }
                        else
                        {
                            $arr2= $this->getCurrentCustomerGroup()->getAllowedPaymentMethods();
                            $key = array_search('checkmo', $arr2); 
                            unset($arr2[$key]);
                            $arr2;
                        }
                        if($arr1 && !$arr2)
