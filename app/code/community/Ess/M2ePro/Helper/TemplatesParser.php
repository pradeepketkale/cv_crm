...
class Ess_M2ePro_Helper_TemplatesParser extends Mage_Core_Helper_Abstract
{
    // ########################################
    ...
    private function insertAttributes($text, Ess_M2ePro_Model_MagentoProduct $magentoProduct)
    {
        ...
        foreach ($matches[1] as $attributeCode) {
            ...
            if (!is_array($value) && $value != '') {
                ...
                } else if (in_array($attributeCode, array('price', 'special_price'))) {
                    $value = Mage::getModel('M2ePro/SellingFormatTemplates')->parsePrice($value, false);
                    $storeId = $magentoProduct->getProduct()->getStoreId();
                    $store = Mage::app()->getStore($storeId);
                    $value = $store->formatPrice($value);
                }
    ...
    private function insertImages($text, Ess_M2ePro_Model_MagentoProduct $magentoProduct)
    {
        ...
        $blockObj = Mage::getSingleton('core/layout')->createBlock('M2ePro/adminhtml_descriptionTemplates_templates_image');
    ...
    private function insertMediaGalleries($text, Ess_M2ePro_Model_MagentoProduct $magentoProduct)
    {
        ...
        foreach ($matches[0] as $key => $match) {
            ...
            $useGalleryView = $realMediaGalleryAttributes[3] == '2' ? true : false;
            $imagesQty = (int)$realMediaGalleryAttributes[5];
			
            if ($imagesQty == 0) { // all
                $imagesQty = $useGalleryView ? 100 : 25;
            }
            ...
            !in_array($realMediaGalleryAttributes[4], array('row', 'column')) && $realMediaGalleryAttributes[4] = 'row';
            ...
            $tempHtml = '';
            $attributeCounter++;
            foreach ($galleryImagesLinks as $key => $imageLink) {
                $imageCounter = '';
                if ($key == 0) {
                    $imageCounter = 'first';
                } else if ($key == count($galleryImagesLinks) - 1) {
                    $imageCounter = 'last';
                }

                $data['image_counter'] = $imageCounter;
