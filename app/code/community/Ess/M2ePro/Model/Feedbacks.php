...
class Ess_M2ePro_Model_Feedbacks extends Mage_Core_Model_Abstract
{
    ...
    public static function receiveFeedbacks(Ess_M2ePro_Model_Accounts $account, array $paramsConnector = array())
    {
        ...
        foreach ($feedbacks as $feedback) {
            ...
            $existFeedback = Mage::getModel('M2ePro/Feedbacks')
                                    ->getCollection()
            ...
            $tempModel = Mage::getModel('M2ePro/Feedbacks');

            if (count($existFeedback['items']) != 0) {
                $tempModel->setId($existFeedback['items'][0]['id']);
                if ($feedback['from_role'] == self::ROLE_BUYER && !$existFeedback['items'][0]['buyer_feedback_id']) {
                    $countNewFeedbacks++;
                }
                if ($feedback['from_role'] == self::ROLE_SELLER && !$existFeedback['items'][0]['seller_feedback_id']) {
                    $countNewFeedbacks++;
                }
            } else {
                $countNewFeedbacks++;
            ...
            $tempModel->addData($dbFeedback)->save();
        }
        //-----------------------
