...
class Ess_M2ePro_Model_ListingsProducts extends Mage_Core_Model_Abstract
{
    ...
    public function getQty($productMode=false)
    {
        if ($this->isListingTypeAuction()) {
            if ($productMode) {
                if ($this->getMagentoProduct()->isVariationOnlyType() &&
                    !$this->getListingTemplate()->isVariationMode()) {
                    return $this->_getVariationOnlyProductQty();
                }
                return $this->getMagentoProduct()->getQty();
            }
            return 1;
        }
        ...
        if ($this->getListingTemplate()->isVariationMode() &&
            ...
            $totalQty = 0;
            foreach ($this->getListingsProductsVariations(true, array('delete' => Ess_M2ePro_Model_ListingsProductsVariations::DELETE_NO)) as $singleVariation) {
                /** @var $singleVariation Ess_M2ePro_Model_ListingsProductsVariations */
                $totalQty += $singleVariation->getQty();
            }
        ...
        switch ($src['mode']) {
            case Ess_M2ePro_Model_SellingFormatTemplates::QTY_MODE_SINGLE:
                if ($productMode) {
                    if ($this->getMagentoProduct()->isVariationOnlyType() &&
                        !$this->getListingTemplate()->isVariationMode()) {
                        $qty = $this->_getVariationOnlyProductQty();
                    } else {
    ...
    protected function _getVariationOnlyProductQty()
    {
    ...
    /**
     * Return QTY for configurable product
     *
     * Get summ qty of all used product that in stock.

     * @return int qty of product
     */
    protected function _getConfigurableProductQty()
    {
        ...
        return floor($totalQty);
    }

    /**
     * Return QTY for grouped product
     *
     * Get summ qty of all used product that in stock.
     * @return int qty of product
     */
    ...
    /**
     * Get Qty for product with type bundle.
     *
     * Get min qty for one of required option.
     * Example.
     * Bundle product have options
     * Option1 => (
     *    var1 => 5 // qty
     *    var2 => 7
     * )
     ...
     * )
     *
     * Total QTY = 5. User can buy 5 different products. For Option2 have only
     * 5 (3+2) possible qty for product. For Option1 => 13 (5 + 7).
     *
     * @return int number of qty available for buy
     *
     * @todo refactor to increase speed of execution. Many sql queries
     */
    ...
    public function getBestOfferAcceptPrice()
    {
        ...
        switch ($src['mode']) {
            case Ess_M2ePro_Model_SellingFormatTemplates::BEST_OFFER_ACCEPT_MODE_PERCENTAGE:
                $price = $this->getBuyItNowPrice() * $src['value'] / 100;
                break;
