...
class Ess_M2ePro_Model_LogsCleaning extends Mage_Core_Model_Abstract
{
	...
	/** Craftsvilla Comment By Amit Pitre On 08-05-2012 as it is taking Read Database Connection to Delete Records in Master-Slave Architecture. So change it with simple query with write DB connection. */
        //$connRead = Mage::getModel('Core/Mysql4_Config')->getReadConnection();
        //$connRead->delete($table, $where);


        //$connRead->delete($table,$where);
		$write = Mage::getSingleton('core/resource')->getConnection('M2ePro_write');
		$deleteQuery = "Delete from ".$table." Where `create_date` < '".(string)$minTime."' OR `create_date` IS NULL";
