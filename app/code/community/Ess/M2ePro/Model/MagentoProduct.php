...
class Ess_M2ePro_Model_MagentoProduct extends Mage_Core_Model_Abstract
{
    ...
    {
        if (!$this->_productModel && $this->_productId > 0) {
            return self::getSkuByProductId($this->_productId);
        }
    ...
    {
        if (!$this->_productModel && $this->_productId > 0) {
            return self::getStockAvailabilityByProductId($this->_productId);
        }
    ...
    {
        if (!$this->_productModel && $this->_productId > 0) {
            return self::getStatusByProductId($this->_productId, $this->_storeId);
        }
    ...
    public function getGalleryImagesLinks($limitImages = 0)
    {
        ...
        foreach ($galleryImages['images'] as $galleryImage) {
            ...
            $images[] = $baseUrl . 'catalog/product' . $galleryImage['file'];
            $i++;
        }
