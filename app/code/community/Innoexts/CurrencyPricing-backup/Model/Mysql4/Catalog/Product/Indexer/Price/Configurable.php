    ...
    extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Indexer_Price_Configurable 
{
    ...
    protected function _applyConfigurableOption()
    {
        ...
        $select = $write->select()
            ...
            ->joinLeft(
                array('cr' => $this->getTable('directory/currency_rate')), 
                '(cr.currency_from = '.$this->getBaseCurrencyExpr('i.website_id').') AND (cr.currency_to = i.currency)', array())
            ->where('le.required_options = 0')
            ->group(array('l.parent_id', 'i.customer_group_id', 'i.website_id', 'l.product_id', 'i.currency'));
        $rate = 'cr.rate';
        $apwPricingValue   = new Zend_Db_Expr('apw.pricing_value');
        $apwPricingValue   = new Zend_Db_Expr($this->getCurrencyPriceExpr(null, $apwPricingValue, $rate));
        $apdPricingValue   = new Zend_Db_Expr('apd.pricing_value');
        $apdPricingValue   = new Zend_Db_Expr($this->getCurrencyPriceExpr(null, $apdPricingValue, $rate));
        $priceExpression   = new Zend_Db_Expr("IF (apw.value_id IS NOT NULL, {$apwPricingValue}, {$apdPricingValue})");
        $priceExpression2  = new Zend_Db_Expr("IF (apw.value_id IS NOT NULL, apw.pricing_value, apd.pricing_value)");
        $percenExpr        = new Zend_Db_Expr('IF (apw.value_id IS NOT NULL, apw.is_percent, apd.is_percent)');
        $roundExpr         = new Zend_Db_Expr("ROUND(i.price * ({$priceExpression2} / 100), 4)");
        $roundPriceExpr    = new Zend_Db_Expr("IF ({$percenExpr} = 1, {$roundExpr}, {$priceExpression})");
        $priceColumn       = new Zend_Db_Expr("IF ({$priceExpression2} IS NULL, '0', {$roundPriceExpr})");
        ...
        $select = $write->select()
            ->from(array($coaTable), array(
                'parent_id', 'customer_group_id', 'website_id', 'MIN(price)', 'MAX(price)', 'MIN(tier_price)', 'currency'))
            ->group(array('parent_id', 'customer_group_id', 'website_id', 'currency'));
        $query = $select->insertFromSelect($copTable);
        $write->query($query);
        $table  = array('i' => $this->_getDefaultFinalPriceTable());
        $select = $write->select()
