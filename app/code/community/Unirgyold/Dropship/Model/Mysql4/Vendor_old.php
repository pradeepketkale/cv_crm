...
class Unirgy_Dropship_Model_Mysql4_Vendor extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getShippingMethods($vendor, $system=false)
    {
        ...
        $select = $read->select()
            ...
            ->joinLeft(array('sm'=>$this->getTable('shipping_method')), "sm.shipping_id=vs.shipping_id and sm.carrier_code=ifnull(vs.carrier_code, '".$vendor->getCarrierCode()."')", array('method_code'))
            ...
            ->columns(array(
                ...
                'ovrd_carrier_code' => 'vs.carrier_code', 'carrier_code'=>new Zend_Db_Expr("ifnull(vs.carrier_code, '".$vendor->getCarrierCode()."')")
            ), 'vs')
        ;
    ...
    {
        $products = $this->getVendorAttributeProducts($vendor, $productIds);
        $products = array_merge($products, $this->getVendorTableProducts($vendor, $productIds));
        return $products;
    }
...
#echo "OLD: "; var_dump($old);
        ...
        foreach ($shipping as $id=>$a) {
            ...
            if (!isset($old[$id])) {
                ...
                $insert[] = '('.(int)$vId.','.(int)$id.",".$a['carrier_code'].",".$a['est_carrier_code'].")";
            }
            // update
...
#echo "INSERT: "; var_dump($insert); exit;
            $write->query("replace into {$table} (vendor_id, shipping_id, carrier_code, est_carrier_code) values ".join(',', $insert));
        }
        if ($delete) {
