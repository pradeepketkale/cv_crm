...
class Unirgy_Dropship_Block_Adminhtml_Report_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _getFlatExpressionColumn($key, $bypass=true)
    {
    	...
    	switch ($key) {
    		case 'tracking_ids':
    			$result = new Zend_Db_Expr("(select group_concat(st.number separator ', ') from {$this->t('sales_flat_shipment_track')} st where parent_id=main_table.entity_id)");
    			break;
    		case 'tax_amount':
    ...
    protected function _prepareCollection()
    {
        if (Mage::helper('udropship')->isSalesFlat()) {
            ...
            $trackingIds = "(select group_concat(stn.value separator ', ') from {$this->t('sales_order_entity_text')} stn inner join {$this->t('sales_order_entity')} st on st.entity_id=stn.entity_id where stn.attribute_id={$stnAttr->getId()} and st.parent_id=e.entity_id)";
