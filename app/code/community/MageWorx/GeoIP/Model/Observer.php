...
class MageWorx_GeoIP_Model_Observer
{
	...
	{
		if (Mage::app()->getStore()->isAdmin() || Mage::registry('geoip_observer_executed')) {
			Mage::unregister('geoip_observer_executed');
			return;
		}
...
//		$customerIp = '62.146.105.136'; // GERMANY
		...
		// Craftsvilla Comment Added By Amit Pitre On 05-07-2012 for Client Ip Address In Load Balancing
		//$customerIp   = $helper->getCustomerIp();
		$customerIp   = $helper->getuserip();
		////////////////////////////////////////////////////////////////////////////////////////////////
		...
		Mage::register('geoip_observer_executed',true);
    		...
    		if (count($stores) > 1) {
    			...
    			if ($customerStoreCode && $customerStoreCode != $currentStoreCode) {
                    if($request->getControllerName() == 'product' && $request->getActionName() == 'view' && ($productId = $request->getParam('id'))){
                        ...
                        return $response->setRedirect($product->getProductUrl());
                    } else {
