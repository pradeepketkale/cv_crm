...
class Phoenix_CashOnDelivery_Model_Observer extends Mage_Core_Model_Abstract 
{
    ...
    {        
		...
		//Added By Amit On (13-Apr-2012)
		$AllVendors = array();
		$_vendorsCODFee = '';
		$_udropHelper = Mage::helper('udropship');
		foreach($quote->getAllVisibleItems() as $items) {
			if($items->getUdropshipVendor()){
				if(in_array($items->getUdropshipVendor(),$AllVendors)) continue;
				else{
					$AllVendors[] = $items->getUdropshipVendor();
					$vendorinfo = $_udropHelper->getVendor($items->getUdropshipVendor());
					$_vendorsCODFee +=$vendorinfo->getCodFee();
				}
			}
		}
		//
        $data = $observer->getInput();
		//Changed By Amit On (13-Apr-2012)
        $quote->setCodFee($_vendorsCODFee!=''?$_vendorsCODFee:0);
        $quote->setBaseCodFee($_vendorsCODFee!=''?$_vendorsCODFee:0);
		//$quote->setCodFee(0);
        //$quote->setBaseCodFee(0);
        $quote->setCodTaxAmount(0);
        $quote->setBaseCodTaxAmount(0);
        foreach ($quote->getAllAddresses() as $address) {
			//Commented By Amit On (13-Apr-2012) as COD Fee is getting added multiple times.
