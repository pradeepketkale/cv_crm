...
(1,24,27,28,30,31)  AND d.courier_name ='Aramex' AND d.number IS NOT NULL
...
foreach($shipmentreport as $_shipmentreport)
{
	...
	echo $vendor_email = $vendor['email'];
	...
	echo $shipmentStatus = $shipment->getUdropshipStatus();
	...
	try {
		if(!empty($awbNumberDel)) $_resAramex = $soapClient->TrackShipments($aramexParams);
	} catch (Exception $e)
	{
	...
	if(is_object($_resAramex) && !$_resAramex->HasErrorsi && !empty($awbNumberDel))
	{
//		print_r($_resAramex); exit;
	...
	$sendEmailFlag = false;
		...
		elseif(($waybillstatus=='Out for Delivery') || ($waybillstatus=='Departed Operations ') || ($waybillstatus=='Attempted Delivery -'))
		{
			...
			{
				echo $subject = 'COD Shipment '.$incrementid.' Is On Its Way To Customer By Our Logistic Service Provider!';
		...
		else{ echo "Not Found Any Status";}
		if($sendEmailFlag)
		{
				echo 'Sending Email';
	...
	echo "\n";
				...
				$summary = "Aramex COD Delivered: ".$totalDelivered.", Shipped: ".$totalShipped.", Picked: ".$totalPicked.", Returned: ".$totalReturned;
