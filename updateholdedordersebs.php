...
$orderDetail = "SELECT * FROM `sales_flat_order` WHERE  `created_at` BETWEEN DATE_SUB(Now(),INTERVAL 12 DAY) AND DATE_SUB(Now(),INTERVAL 2 HOUR) AND `status` = 'holded'";
$resultorderDetail = $db->query($orderDetail,$readId)->fetchAll();
$k = 0;
$numproc = 0;
$numcan = 0;
...
if($k < 25)
{
	...
	$resRefLink = $db->query($refQuery,$readId);
