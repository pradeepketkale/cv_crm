...
$channel = $connection->channel();
        $channel->basic_qos(
            ...
            10,     #prefetch count - prefetch window in terms of whole messages
...
//$msg->delivery_info['channel']->basic_ack($msg->delivery_info['delivery_tag']);return;
	
	$solr=getSolrConnection();
...
$document->special_price = $cattext['61']>0?$cattext['61']:$document->price;
...
$catid2 = array();
	...
	foreach ($catIds as $category_id){
		...
		$document->category_name = $catid2;
				...
				$document->category_name = 'Online Shopping';
				$document->category_id = 50000;
			}
...
tryagain:
	try {
		...
		echo "solr logs".$e->getMessage();goto tryagain;
	}


};

$channel->basic_consume('push_to_solr_q', '', false, false, false, false, $callback);
