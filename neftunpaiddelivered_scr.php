	...
	foreach($trackdetailsShippedCusFedexRes as $_trackdetailsShippedCusFedexRes) 
	{
		...
		$shippedToCusFedex[] = array($shipmentid, $updatedat, $amount, $trackNumberFedex);
				...
				fclose($fpShippedFed);
					...
					$body = '<a href="http://175.41.147.59/media/vendorcsv/merchantIds.csv">click here to download the Merchant IDS</a>'.'<br>';
					$body .= '<a href="http://175.41.147.59/media/vendorcsv/unpaid_delivered_aramex.csv">click here to download the Unpaid Delivered Aramex</a>'.'<br>';
					$body .= '<a href="http://175.41.147.59/media/vendorcsv/unpaid_delivered_fedex.csv">click here to download the Unpaid Delivered Fedex</a>'.'<br>';
					$body .= '<a href="http://175.41.147.59/media/vendorcsv/unpaid_shipped_to_customer_aramex.csv">click here to download the Unpaid Shipped To Customer Aramex</a>'.'<br>'; 
					$body .= '<a href="http://175.41.147.59/media/vendorcsv/unpaid_shipped_to_customer_fedex.csv">click here to download the Unpaid Shipped To Customer Fedex</a>';
					
					$mail = Mage::getModel('core/email');
					$mail->setToName('Craftsvilla Places');
					$mail->setToEmail('manoj@craftsvilla.com');
					$mail->setBody($body);
					$mail->setSubject("Craftsvilla Unpaid Report: COD for Fedex and Aramex");
					$mail->setFromEmail('places@craftsvilla.com');
					$mail->setFromName("Payment Stats");
					$mail->setType('html');
	
					if($mail->send())
					{
					 $mail->setToEmail('accounts@craftsvilla.com');
					$mail->send();
