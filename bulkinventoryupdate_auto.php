...
//echo $bulkid = $argv[42]; 
//echo $bulkid = $argv[1];
$bulkuploadIds = Mage::getModel('bulkinventoryupdate/bulkinventoryupdate')->getCollection();
                $bulkuploadIds->getSelect()->where('main_table.status = 5');
echo 'Total Submissions: '.$bulkuploadIds->count().'\n';
$i = 0;
$n = 0;
foreach($bulkuploadIds as $bulkinventoryupdate)
{
        echo 'Starting Loop '.$n;
	if($n < 10)
	{
		echo $bulkid = $bulkinventoryupdate->getId();
		...
		foreach ($csvData as $k => $v) 
		{
			...
			if (($i> 0) && (strlen($v[0]) > 2) && ($i < 6000))
			{
				...
				if(empty($sku) || $qty=='' || ($qty<0) || !is_numeric($qty))
				{  
	...
	} // end of if for external loop
	$n++;
} // end of foreach on bulkids
?>
