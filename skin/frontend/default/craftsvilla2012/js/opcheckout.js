...
this.progressUrlorder = urls.progress;
 ...
 reloadProgressBlockOrder: function(){
        var updater = new Ajax.Updater('checkout-progress-wrapper-order', this.progressUrlorder, {method: 'get', onFailure: this.ajaxFailure.bind(this)});
    },
    ...
    setBilling: function() {
        ...
        this.reloadProgressBlock();this.reloadProgressBlockOrder();
...
Billing.prototype = {
    ...
    setAddress: function(addressId){
        if(!this.addressUrl)
        {
        	//this.addressUrl = "http://doejofinal.craftsvilla.com/checkout/onepage/getAddress/address/";
        	this.addressUrl = "/checkout/onepage/getAddress/address/";
        }
        ...
        if (addressId) {
            request = new Ajax.Request(
                ...
                {method:'get', onSuccess: function(transport) {
                		...
                		{
                			document.getElementById("billing:firstname").value=myObject.firstname;
                		}	
                		if(myObject.lastname)
                		{
                			document.getElementById("billing:lastname").value=myObject.lastname;
                		}
                		if(myObject.company)
                		{
                			document.getElementById("billing:company").value=myObject.company;
                		}
                		if(myObject.street1)
                		{
                			document.getElementById("billing:street1").value=myObject.street1;
                		}
						if(myObject.street2)
						{
							document.getElementById("billing:street2").value=myObject.street2;
						}
						document.getElementById("billing:country_id").value=myObject.country_id;
						
						if(myObject.region_id)
						{
							document.getElementById('billing:region').style.display = 'none';
							document.getElementById('billing:region_id').style.display = 'block';
							billingRegionUpdater = new RegionUpdater('billing:country_id', 'billing:region', 'billing:region_id', countryRegions, undefined, 'billing:postcode');
							billingRegionUpdater.update();
							document.getElementById("billing:region").value='';
							document.getElementById("billing:region_id").value=myObject.region;
						}
						else
						{
							document.getElementById('billing:region_id').style.display = 'none';
							document.getElementById('billing:region').style.display = 'block';
							document.getElementById("billing:region").value=myObject.region;
							document.getElementById("billing:region_id").value='';
						}
						$('billing:region_id').setAttribute('defaultValue',  myObject.region_id);
						//document.getElementById("billing:region").value=myObject.region;
						if(myObject.city)
                		{
							document.getElementById("billing:city").value=myObject.city;
                		}
						if(myObject.postcode)
                		{
							document.getElementById("billing:postcode").value=myObject.postcode;
                		}
						if(myObject.telephone)
                		{
							document.getElementById("billing:telephone").value=myObject.telephone;
                		}
						if(myObject.fax)
                		{
							document.getElementById("billing:fax").value=myObject.fax;
                		}
                	}
    ...
    newAddress: function(isNew){
        ...
        if (isNew) {
			...
			document.getElementById("billing:country_id").value='';
            ...
            Billing.prototype.setAddress(adr_id_val);
        }
    },
...
ShippingMethod.prototype = {
    ...
    nextStep: function(transport){
        ...
        if (response.goto_section) {
            ...
            checkout.reloadProgressBlock();checkout.reloadProgressBlockOrder();
            return;
        }
